...
std::vector<int> convertMatrixToVector(const gsMatrix<real_t> &mat) {
    std::vector<int> vec;
    index_t DIM = 1;
    vec.reserve(mat.size());
    for (index_t i = 0; i < DIM; ++i) {
        for (index_t j = 0; j < mat.cols(); ++j) {
            vec.push_back(static_cast<int>(mat(i, j)));
        }
    }
    return vec;
}
...
int main(int argc, char *argv[])
{
    ...
    gsMatrix<> coefColsMatrix, knotColsMatrix;
    gsVector<index_t> geometryKnotColsIDs, geometryControlColsIDs;

    participant.getMeshVertexIDsAndCoordinates(GeometryKnotMeshCols, geometryKnotColsIDs, knotColsMatrix);
    participant.getMeshVertexIDsAndCoordinates(GeometryControlMeshCols, geometryControlColsIDs, coefColsMatrix);


    // gsBasis<> * geometryKnotBasis = knotMatrixToBasis<real_t>(geometryKnots).get();

    std::vector<int> knotColsVec = convertMatrixToVector(knotColsMatrix);
    std::vector<int> coefColsVec = convertMatrixToVector(coefColsMatrix);
