...
int main(int argc, char *argv[])
{
    ...
    //Generate domain
    std::string filenameSolid = "flappingBeam_beam.xml";
    gsMultiPatch<real_t> patches;
    gsReadFile<real_t>( filenameSolid, patches);

    // gsWriteParaview<>( patches, "beam_mesh", 1000);
    // return 0;
    ...
    couplingInterfaces[1] = patchSide(0,boundary::east);
    couplingInterfaces[2] = patchSide(0,boundary::north);
...
//----------------------------------------------------------------------------------------------
    ...
    bcInfo.addCondition(0, boundary::west, condition_type::dirichlet, &g_D, -1);
    // bcInfo.addCondition(0, boundary::west, condition_type::dirichlet, &g_D, 1);
    // Assign geometry map
