...
//    gsMatrix<> xy2(patches.targetDim(),quadSize);
    ...
    gsPreCICE<real_t> interface("Solid-Downstream", precice_config);
    interface.addMesh("Solid-Mesh-Downstream",xy);
    real_t precice_dt = interface.initialize();

    index_t meshID = interface.getMeshID("Solid-Mesh-Downstream");
    index_t dispID = interface.getDataID("Displacement-Downstream",meshID);
    index_t forceID = interface.getDataID("Stress-Downstream",meshID);
