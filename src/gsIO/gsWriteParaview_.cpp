...
void gsWriteParaview(const gsGeometry<T> & Geo, std::string const & fn,
                     unsigned npts, bool mesh, bool ctrlNet, bool skipPvd, unsigned precision);
...
void gsWriteParaviewUnstructuredGrid(const gsMultiPatch<T> & mPatch, std::string const & fn,
                                     unsigned npts, bool export_base64, bool skipPvd, unsigned precision,
                                     bool blockColors);
...
TEMPLATE_INST
void gsWriteParaview(gsGeometrySlice<T> const& Geo, std::string const & fn, unsigned npts, unsigned precision );

TEMPLATE_INST
void gsWriteParaview(gsFunctionSet<T> const& func, std::string const & fn, unsigned npts, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(gsFunctionSet<T> const& geo, gsFunctionSet<T> const& func,std::string const & fn, unsigned npts, const std::string pDelim, bool skipPvd, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(const gsMultiPatch<T> & Geo, std::string const & fn,
                     unsigned npts, bool mesh, bool ctrlNet, const std::string pDelim, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(gsMappedSpline<2,T> const& mspline, std::string const & fn,unsigned npts, bool skipPvd, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(gsFunctionSet<T> const& geom, gsMappedBasis<2,T>  const& mbasis,std::string const & fn,unsigned npts,const bool fullsupport, const std::vector<index_t> indices, bool skipPvd, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(gsMultiPatch<T> const& mp, gsMultiBasis<T> const& mb,std::string const & fn, unsigned npts, bool skipPvd, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(gsFunction<T> const& func, gsMatrix<T> const& supp, std::string const & fn, unsigned npts, bool graph, unsigned precision);
...
TEMPLATE_INST
void gsWriteParaview(const gsMatrix<T> & box, std::string const & fn, const gsVector<T> & values, unsigned precision);
TEMPLATE_INST
void gsWriteParaview(const gsMatrix<T> & box, std::string const & fn, const std::vector<T> & values, unsigned precision);
TEMPLATE_INST
void gsWriteParaview(const gsMatrix<T> & box, std::string const & fn, T value, unsigned precision);
...
TEMPLATE_INST
void gsWriteParaviewPoints(gsMatrix<T> const& X, gsMatrix<T> const& Y, std::string const & fn, unsigned precision);

TEMPLATE_INST
void gsWriteParaviewPoints(gsMatrix<T> const& X, gsMatrix<T> const& Y, gsMatrix<T> const& z, std::string const & fn, unsigned precision);

TEMPLATE_INST
void gsWriteParaviewPoints(gsMatrix<T> const& X, gsMatrix<T> const& Y, gsMatrix<T> const& z, gsMatrix<T> const& v, std::string const & fn, unsigned precision);

TEMPLATE_INST
void gsWriteParaviewPoints(gsMatrix<T> const& points, std::string const & fn, unsigned precision);
...
void gsWriteParaviewSolid(gsSolid<T> const  & sl,
                     std::string const & fn,
                     unsigned numSamples, unsigned precision );
...
TEMPLATE_INST
void gsWriteParaview(gsMesh<T> const& sl, std::string const & fn, const gsMatrix<T>& params, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(const gsSurfMesh<T> & sm,
                     std::string const & fn, unsigned precision);

TEMPLATE_INST
void gsWriteParaview(const gsSurfMesh<T> & sm,
                     std::string const & fn,
                     std::vector<std::string> props, unsigned precision);
...
TEMPLATE_INST
void gsWriteParaview(const std::vector<gsMesh<T> >& sl, std::string const & fn, unsigned precision);
...
TEMPLATE_INST
void writeSingleBasisMesh(const gsBasis<T> & basis, std::string const & fn, unsigned precision);

TEMPLATE_INST
void writeSingleCompMesh(const gsBasis<T> & basis, const gsGeometry<T> & Geo,
                         std::string const & fn, unsigned resolution, unsigned precision);

TEMPLATE_INST
void writeSingleBox(const gsMatrix<T> & box, std::string const & fn, T value, unsigned precision);

TEMPLATE_INST
void writeSingleHBox(const gsHBox<2,T> & box, std::string const & fn, unsigned precision);

TEMPLATE_INST
void writeSingleHBox(const gsHBox<3,T> & box, std::string const & fn, unsigned precision);

TEMPLATE_INST
void writeSingleControlNet(const gsGeometry<T> & Geo, std::string const & fn, unsigned precision);
