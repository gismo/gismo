...
template <class T>
gsRemapInterfaceOld<T>::gsRemapInterfaceOld(const gsMultiPatch<T>   & mp,
...
template<class T>
const std::vector<T> gsRemapInterfaceOld<T>::getPointsOnInterface() const
{
...
template<class T>
void gsRemapInterfaceOld<T>::constructBreaks() {
...
template<class T>
void gsRemapInterfaceOld<T>::constructReparam()
{
...
template <typename T>
void gsRemapInterfaceOld<T>::eval_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
...
template<class T>
memory::unique_ptr< gsDomainIterator<T> > gsRemapInterfaceOld<T>::makeDomainIterator() const
{
...
template<class T>
void gsRemapInterfaceOld<T>::enrichToVector(const short_t         boundarySide,
...
template<class T>
void gsRemapInterfaceOld<T>::findInterface()
{
...
template<class T>
void gsRemapInterfaceOld<T>::findInterface(const boundaryInterface& bi)
{
    //const gsGeometry<T> & patch1 = m_domain.patch(0); // m_g1
    //const gsGeometry<T> & patch2 = m_domain.patch(1); // m_g2
...
template<class T>
bool gsRemapInterfaceOld<T>::checkIfMatching()
{
...
template <class T>
gsMatrix<T> gsRemapInterfaceOld<T>::checkIfInBound(const gsMatrix<T> & u) const
{
...
template<typename T>
void gsRemapInterfaceOld<T>::changeDir(const boundaryInterface & bi)
{
    T tmp = 0;
    index_t row = -1;
