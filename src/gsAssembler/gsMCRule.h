...
*/
template<class T>
class gsMCRule GISMO_FINAL : public gsQuadRule<T>
{
public:

    typedef memory::unique_ptr<gsMCRule> uPtr;

    /// Default empty constructor
    gsMCRule() { }

    /// Initialize a tensor-product Gauss quadrature rule with \a numNodes (direction-wise)
    gsMCRule(gsVector<index_t> const & numNodes,
                const unsigned digits = 0 )
    {
        gsMCRule::setNodes(numNodes, digits);
    }

    /// Make function returning a smart pointer
    static uPtr make(gsVector<index_t> const & numNodes,
                        const unsigned digits = 0 )
    { return uPtr( new gsMCRule(numNodes,digits) ); }

    /// Initialize a 1D Gauss quadrature rule with \a numNodes
    gsMCRule(index_t numNodes, const unsigned digits = 0 )
    {
    ...
    gsMCRule(const gsBasis<T> & basis, const T quA, const index_t quB, short_t fixDir = -1);
    //const unsigned digits = std::numeric_limits<T>::digits10 );
    gsMCRule(const gsDomain<T> & domain, const T quA, const index_t quB, short_t fixDir = -1);

    /// Initialize a tensor-product Gauss quadrature rule for \a basis
    /// using quA *deg_i + quB nodes (direction-wise). Values of quA
    /// and quB are taken from the \a options
    gsMCRule(const gsBasis<T> & basis, const gsOptionList & options, short_t fixDir = -1);
    //const unsigned digits = std::numeric_limits<T>::digits10 );

    gsMCRule(const gsDomain<T> & domain, const gsOptionList & options, short_t fixDir = -1);
...
#include GISMO_HPP_HEADER(gsMCRule.hpp)
#endif
