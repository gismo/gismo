...
public:
    ...
    : Base(_discSolution,_rhsFunction), m_bcInfo(bcInfo), m_f2param(_rhsFunctionParam)
    {
    ...
    gsErrEstPoissonResidual(const gsField<T> & _discSolution,
             const gsFunction<T> & _rhsFunction,
             bool _rhsFunctionParam = false)
    : Base(_discSolution,_rhsFunction), m_f2param(_rhsFunctionParam)
    {
...
protected:
     ...
     *
     * Sets up the quadrature rule (based on the degree of \em basis)
     * that are needed for this specific problem.
     *
     * \param[in] basis
     * \param[out] rule
     */
     ...
     * The const gsGeometry \em geo is also evaluated at the nodes,
                         ...
                         gsMatrix<T>         & quNodes)
    {
        ...
        discSolution.deriv_into(md.points, m_discSolDer);
        discSolution.deriv2_into(md.points, m_discSol2ndDer);

        // Compute geometry related values
        geo.computeMap(md);

        // Evaluate right-hand-side function (defined of physical domain)
        rhsFunction.eval_into(md.values[0], m_rhsFctVals);
}
     ...
     * @param[in] geo const gsGeometry as evaluated in evaluate().
                     ...
                     T & accumulated)
    {
        ...
        for (index_t k = 0; k < quWeights.size(); ++k) // loop over quadrature nodes
        {
            ...
            //const typename gsMatrix<T>::constColumns J = md.jacobian(k);
//            gsMatrix<T> sol_der2 = m_discSol2ndDer.col(k); // not used

            transformLaplaceHgrad(md, k, m_discSolDer, m_discSol2ndDer, m_phLaplace);
//            transformGradients(md, k, m_discSolDer , m_phdiscSolDer); // not used
        ...
        for (size_t psi = 0; psi < touchingSides.size(); psi++) // todo: use getConditionsForPatch and iterate over them
        {
            ...
            if (isIntfc)
            {
                sumSidesSq += 0.5 * diffIntfc(*field1, geo, ps, intfcRes, quNodesSide, quWeightsSide);
            }
            else
            {
                // if bc is the null-pointer, the side will be considered a free surface, i.e.,
                // one with homogenous Neumann bondary conditions.
                sumSidesSq += diffNeumannBC(*field1, geo, ps, bc, quNodesSide, quWeightsSide);
            }

        } // psi

        // Estimate the cell-size on the physical domain
        T hhSq = cellsizeEstimateSquared(element, geo);
...
public:
    ...
    {
        md.points = quNodes;
        const size_t d = size_t(md.points.rows());
        ...
        gsMapData<T> mdNeigh;
        mdNeigh.points = md.points;
        mdNeigh.points.setZero();
        for (size_t k = 0; k < size_t(md.points.cols()); k++)
        {
            for (size_t i = 0; i < d; i++)
            {
                mdNeigh.points(intfc.dirMap(ps, i), k) = md.points(i, k);
                if (!intfc.dirOrientation(ps, i))
                    mdNeigh.points(intfc.dirMap(ps, i), k) = T(1.0) - mdNeigh.points(intfc.dirMap(ps, i), k);
            }
        }
        ...
        mdNeigh.flags = md.flags;
        const gsGeometry<T> & geoNeigh = patchesPtr->patch(psNeigh.patch);
        geoNeigh.computeMap(mdNeigh);
        ...
        solField.igaFunction(ps.patch).deriv_into(md.points, grads);
        solField.igaFunction(psNeigh.patch).deriv_into(mdNeigh.points, gradsNeigh);

        gsVector<T> outerNorm;

        T sum(0.0);
        // quNodes
        for (size_t qk = 0; qk < size_t(md.points.cols()); qk++)
        {
            const T weight = quWeights[qk] * md.measure(qk);

            // compute normal vector and normalize to 1
            outerNormal(md, qk, ps.side(), outerNorm);
            outerNorm.normalize();

            // transform the gradients
            transformGradients(md, qk, grads, trf_grads);
            transformGradients(mdNeigh, qk, gradsNeigh, trf_gradsNeigh);
                           ...
                           const gsVector<T>           & quWeights)
    {
        ...
        for (size_t qk = 0; qk < size_t(md.points.cols()); qk++)
        {
            ...
            else if (bc->type() == condition_type::neumann)
            {
                gsDebugVar(bc->type());
                gsMatrix<T> bcFct;
                bc->function()->eval_into(md.points.col(qk), bcFct);
                int unk = bc->unknown();
     ...
     */
    inline T cellsizeEstimateSquared( const gsDomainIterator<T> & element, const gsGeometry<T> & geo ) //const
    {
        T hhSq = T(0.0);
        index_t idxMax(0);
        ...
        {
            idxMax = 4;
            md.points = gsMatrix<T>(2, idxMax);

            md.points << element.lowerCorner()[0], element.lowerCorner()[0], \
        ...
        {
            idxMax = 8;
            md.points = gsMatrix<T>(3, idxMax);

            md.points << element.lowerCorner()[0], \
