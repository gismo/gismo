...
*/
template <class T>
class gsJumpAssembler : public gsAssembler<T>
{
public:
...
public:
    ...
    \ingroup Assembler
    */
    gsJumpAssembler( gsMultiPatch<T> const            & patches,
                        gsMultiBasis<T> const         & basis,
                        gsBoundaryConditions<T> const & bconditions,
                        const gsFunction<T>           & rhs,
                        dirichlet::strategy           dirStrategy,
                        typename gsMultiPatch<T>::const_iiterator interface)
    : m_it(interface)
    {
        m_options = defaultOptions();
        m_options.setInt("DirichletStrategy", dirStrategy);
        m_options.setInt("InterfaceStrategy", iFace::dg);
    ...
    {
        return new gsJumpAssembler<T>(*this);
    }
    
    virtual gsAssembler<T>* create() const
    {
        return new gsJumpAssembler<T>();
    }
...
protected:
    typename gsMultiPatch<T>::const_iiterator m_it;
...
#include GISMO_HPP_HEADER(gsJumpAssembler.hpp)
#endif
