...
template <class T>
class gsRemapInterfaceOld : public gsFunction<T>
{
public:
    /// Shared pointer for gsRemapInterfaceOld
    typedef memory::shared_ptr< gsRemapInterfaceOld > Ptr;

    /// Unique pointer for gsRemapInterfaceOld
    typedef memory::unique_ptr< gsRemapInterfaceOld > uPtr;
    ...
    gsRemapInterfaceOld(const gsMultiPatch<T> & mp, const gsMultiBasis<T> & basis, const boundaryInterface & bi);

    /// Constructor for the class which takes two geometries
    gsRemapInterfaceOld(const gsGeometry<T> & g1, const gsGeometry<T> & g2, const gsBasis<T>* basis1, const gsBasis<T>* basis2) : m_g1(g1), m_g2(g2), m_b1(*basis1), m_b2(*basis2)
    {
...
////// THESE TWO FUNCTIONS ARE NEW, IN ORDER TO ALLOW GOOD DEBUGING:
public:
...
}; // End gsRemapInterfaceOld

template <class T>
inline std::ostream & operator<<(std::ostream & os, const gsRemapInterfaceOld<T> & remapIf)
{ return remapIf.print(os); }

} // End namespace gismo

#ifndef GISMO_BUILD_LIB
#include GISMO_HPP_HEADER(gsRemapInterfaceOld.hpp)
#endif
