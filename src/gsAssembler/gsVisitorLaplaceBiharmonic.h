/** @file gsVisitorLaplaceBiharmonic.h
 ...
 */
template <class T>
class gsVisitorLaplaceBiharmonic
{
public:

    gsVisitorLaplaceBiharmonic(const gsPde<T> & , const boundary_condition<T> & s)
    : neudata_ptr( s.function().get() ), side(s.side())
    { }

    gsVisitorLaplaceBiharmonic(const gsFunction<T> & neudata, boxSide s) :
