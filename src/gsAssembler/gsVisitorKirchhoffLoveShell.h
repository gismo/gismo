/** @file gsVisitorKirchhoffLoveShell.h

    @brief Visitor for a simple Kirchhoff-Love equation.
...
/** \brief Visitor for the Kirchhoff-Love equation.
 *
...
template <class T>
class gsVisitorKirchhoffLoveShell
{
public:

    gsVisitorKirchhoffLoveShell(const gsPde<T> & pde)
    {
        rhs_ptr = static_cast<const gsKirchhoffLoveShellPde<T>&>(pde).rhs() ;
    }


    /** \brief Constructor for gsVisitorKirchhoffLoveShell.
     *
     * \param[in] rhs Given right-hand-side function/source term that, for
     */
    gsVisitorKirchhoffLoveShell(const gsFunction<T> & rhs) :
        rhs_ptr(&rhs)
    {
