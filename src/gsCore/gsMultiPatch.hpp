...
template<class T>
gsDofMapper gsMultiPatch<T>::getBoxMapper() const
{
    gsMatrix<index_t> bdr1(2,1), bdr2(2,1); // indices of the boundary control points

    // Create a map which assigns to all meeting patch-local indices a
    // unique global id
    const index_t sz = m_patches.size();
    gsVector<index_t> patchSizes(sz);
    patchSizes.setConstant(4);
    gsDofMapper mapper(patchSizes);
    
    for ( const_iiterator it = iBegin(); it != iEnd(); ++it ) // for all interfaces
    {
        // Grab corner points in matching configuration
        const index_t d1 = it->first() .direction();
        const index_t d2 = it->second().direction();
        const gsVector<bool>    & dirOr = it->dirOrientation();
        const gsVector<index_t> & bMap  = it->dirMap();

        const index_t s1 = it->first() .side();
        const index_t s2 = it->second().side();
        // 1 -- 2 -- 3 -- 4
        // 0,2 -- 1,3 -- 0,1 -- 3,4
        bdr1.at(0) = 0;
        bdr1.at(1) = 1;
    ...
    // Finalize the mapper. At this point all patch corners are mapped
    // to unique global indices
    mapper.finalize();
    return mapper;
}
