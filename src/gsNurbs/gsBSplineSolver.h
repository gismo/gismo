...
public:
    ...
    gsBSplineSolver() : m_n(1),m_k(1), m_d(1), eps( std::max(1e-7,  std::pow(10.0, -(math::numeric_limits<T>::digits10()-1))) ) { }
...
public:
    ...
    bool firstRoot(gsBSpline<T> const & bsp, int const & coord = 0,
                   T const & tr = 0, T const & tol = std::max(1e-7,  std::pow(10.0, -(math::numeric_limits<T>::digits10()-1))), unsigned const & N=100)
    {
