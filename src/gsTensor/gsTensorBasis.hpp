                                            ...
                                            gsMesh<T> & mesh) const
{
    ...
    for (dim_t i = 0; i < d; ++i) // For all axes
    {
        s      = stride(i);
        v      = low;
        upp    = end;
...
/*
template <dim_t d, class BB, class B>
                                          ...
                                          std::vector<gsMatrix<T> >& result) const
{
    ...
    do // for all basis functions
    {
        ...
        for ( dim_t i=1; i!=d; ++i) // for all variables
    ...
    if ( n>=1)
    {
        ...
        do // for all basis functions
        {
            for ( dim_t k=0; k<d; ++k) // for partial derivatives
            {
        ...
        for (int i = 3; i <=n; ++i) // for all orders of derivation
        {
            ...
            do // for all basis functions
            {
                ...
                do // for all partial derivatives of order \a i
                {
                    ...
                    for (dim_t k = 1; k!=d; ++k) // for all variables
                                   ...
                                   gsMatrix<T>& result)
{
    ...
    {
        unsigned m = d;
        for ( dim_t k=0; k<d; ++k)// First compute the pure second derivatives
        {
            index_t cur = r + k;
            ...
            for ( dim_t l=k+1; l<d; ++l) // Then all mixed derivatives follow in lex order
            {
                cur = r + m;
                ...
                for ( dim_t i=0; i<k; ++i)
                    result.row(cur).array() *= values[i][0].row( v.at(i) ).array() ;
                for ( dim_t i=k+1; i<l; ++i)
                    result.row(cur).array() *= values[i][0].row( v.at(i) ).array();
                for ( dim_t i=l+1; i<d; ++i)
                    result.row(cur).array() *= values[i][0].row( v.at(i) ).array() ;
                ++m;
            }
        }
...
gsTensorBasis<d,T>::interpolateAtAnchors(gsMatrix<T> const& vals) const
{
    ...
    for (dim_t i = 0; i < d; ++i) // for all coordinate bases
