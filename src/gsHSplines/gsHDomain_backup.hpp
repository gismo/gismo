...
template<short_t d, class T > void
gsHDomain<d,T>::alignBox(box & iBox, int lvl) const
{
    const unsigned h = 1 << (m_indexLevel - lvl) ;
    index_t cc;
    for ( short_t i = 0; i!=d; ++i )
    {
        iBox.first[i] -= iBox.first[i] % h;//floor
        cc = iBox.second[i] % h;
        iBox.second[i] += (cc ? h-cc : 0 ) ;// ceil
    }
}

template<short_t d, class T > bool
gsHDomain<d,T>::isAligned(point & pt, int lvl) const
{
    const unsigned h = 1 << (m_indexLevel - lvl) ;
    for ( short_t i = 0; i!=d; ++i )
        if ( pt[i] % h != 0 ) return false;
    return true;
}

template<short_t d, class T > std::list<kdnode<d,T>*>
gsHDomain<d,T>::complement(const box & ibox, node * nd) const
{
    std::list<node*> cmpl;
    box tmp(ibox);
    bool flag;
    ...
    while ( ! stack.empty() )
    {
        ...
        if ( curNode->isLeaf() ) // reached a leaf
        {
            ...
            for ( short_t i = 0; i < d; ++i )
            {
                if ( ibox    .first[i]>curNode->box->second[i] ||
                     curNode->box->first[i]>    ibox.second[i] )
                {
                    flag = false;
                    break;
                }
            ...
            //get complement
            // modify node
            tmp.first = ibox.first.cwiseMin(curNode->box->first);
            cmpl.push_back(curNode);
        }
        else // roll down the tree
        {
                            ...
                            int lvl) // CONSTRAINT: lvl is "minimum level"
{
    ...
    gsWarn<<"Unref "<<  k1.transpose()<< " ~ "<<  k2.transpose() <<" at level" <<lvl<<".\n";
    ...
    while ( ! stack.empty() )
    {
        ...
        if ( curNode->isLeaf() ) // reached a leaf
        {
            ...
            gsInfo<<"at leaf "<< curNode<<"\n";
            // If this leaf is coarser than lvl, then we have nothing to do
            if ( curNode->level <= lvl )
                continue;
            ...
            gsInfo<<"Nodes: "<< newLeaf <<" , "<<curNode<<"\n";
            
            // If curNode is still a leaf, its domain is
            // contained in iBox
            if ( !newLeaf ) //  curNode->isLeaf()
            {
                gsInfo<<"No split. Cur node: "<< curNode<<"\n";
            ...
            {
                gsInfo<<"Splitting at "<< (newLeaf->parent->pos>> (m_indexLevel-1)) <<", axis "<<
                    newLeaf->parent->axis<<"\n";
                stack.push_back(newLeaf);
            }
        }
        ...
        {
            gsInfo<<"Internal node "<< (curNode->pos>> (m_indexLevel-1)) <<", axis "<<
                curNode->axis<<"\n";
            gsDebugVar(*curNode);
            ...
            if ( iBox.second[curNode->axis] <= curNode->pos)
            {
                // iBox overlaps only left child of this split-node
                stack.push_back(curNode->left);
                curNode->box->second[curNode->axis]= curNode->pos;
            }
            else if  ( iBox.first[curNode->axis] >= curNode->pos)
            {
                // iBox overlaps only right child of this split-node
                stack.push_back(curNode->right);
                curNode->box->first[curNode->axis]= curNode->pos;
            }
            else
            {
                gsDebugVar(curNode->box->first. transpose());
                gsDebugVar(curNode->box->second.transpose());
                // iBox overlaps both children of this split-node:
                if (0 ==curNode->pos % h)
                {   // the split is aligned to the insertion level
                    stack.push_back(curNode->left ); 
                    stack.push_back(curNode->right);
                }
                else
                {
                    gsDebugVar(iBox.first .transpose());
                    gsDebugVar(iBox.second.transpose());

                    //grab the subtree starting at curNode

                    //replace it with a new node of level lvl
                    // insert subtree leaves excluding iBox
                    curNode->box   = &curBox;
                    curNode->axis  = -1;
                    curNode->level = lvl;

                    gsDebugVar(*curNode->left);
                    if (curNode->left->isLeaf() )
                    {
                        // next loop will split it
                        // BUT is it aligned.. ??
                        stack.push_back(curNode->left);
                    }
                    else
                    {
                        GISMO_ENSURE( curNode->pos % h == 0      
                                      , "LEFT Node position "<<curNode->pos<<" is not aligned to level "<< lvl << " ++ "<< (curNode->pos >> (m_indexLevel-1))
                                  << ". Box interval ["<<iBox.first[curNode->axis]
                                  <<","<<iBox.second[curNode->axis]<<"]" );

                        //there is a split that is not aligned to iBox
                        
                    }
                    
                    gsDebugVar(*curNode->right);
                    if (curNode->right->isLeaf() )
                        stack.push_back(curNode->right); 
                    else
                    {
                        GISMO_ENSURE( curNode->pos % h == 0
                                      , "RIGHT Node position "<<curNode->pos<<" is not aligned to level "<< lvl << " ++ "<< (curNode->pos >> (m_indexLevel-1))
                                  << ". Box interval ["<<iBox.first[curNode->axis]
                                  <<","<<iBox.second[curNode->axis]<<"]" );
...
template<short_t d, class T> inline void
gsHDomain<d,T>::printNodes() const
