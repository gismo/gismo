...
private:
    ...
    void alignBox(box & iBox, int lvl) const;
    bool isAligned(point & pt, int lvl) const;

    std::list<node*> complement(const box & ibox, node * nd) const;
...
private:
    ...
    struct printNodes_visitor
    {
        ...
        static void visitNode(kdnode<d,T> * nd, return_type & i)
        {
            gsInfo<< *nd;
        }
    };
