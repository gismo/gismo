...
// gsDeprecated<d,T>::gsDeprecated(gsBSplineBasis<T> &  bsbasis)
...
// template<short_t d, class T>
// typename gsDeprecated<d,T>::BoundaryBasisType * gsDeprecated<d,T>::basisSlice(index_t dir_fixed,T par ) const
// {
...
//     typename gsDeprecated<d,T>::BoundaryBasisType* bBasis =
//             new typename gsDeprecated<d,T>::BoundaryBasisType(*bBSplineBasis);
...
// template<short_t d, class T>
// std::ostream & gsDeprecated<d,T>::print(std::ostream &os) const
// {
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::initialize()
// {
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::evalSingle_into(index_t i, const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::derivSingle_into(index_t i, const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
//     int lvl = this->levelOf(i);
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::deriv2Single_into(index_t i, const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
//     int lvl = this->levelOf(i);
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::eval_into(const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
...
//         gsDeprecated<d,T>::active_into(curr_u, act);
        
//         // for all actives on the point
//         for(index_t i = 0; i != act.rows(); i++) 
//         {
//             gsDeprecated<d,T>::evalSingle_into( act(i,0), curr_u, curr_res );
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::deriv_into(const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
//     gsVector<index_t> nact;
//     gsMatrix<index_t> act;
//     // Get the number of active functions
...
//             for(index_t i = 0; i < act.rows(); i++) 
//             {
//                 gsDeprecated<d,T>::derivSingle_into( act(i,0), curr_u, curr_res );
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::deriv2_into(const gsMatrix<T> & u, gsMatrix<T>& result) const
// {
//     gsVector<index_t> nact;
//     gsMatrix<index_t> act;
...
//         for(index_t i = 0; i < act.rows(); i++) 
//         {
//             gsDeprecated<d,T>::deriv2Single_into( act(i,0), curr_u, curr_res );
...
// template<short_t d, class T>
// void gsDeprecated<d,T>::transferbyLvl (std::vector<gsSparseMatrix<T> >& result)
// {
//     result.clear();
...
// template<short_t d, class T>
// gsSparseMatrix<T> gsDeprecated<d,T>::coarsening( const std::vector<gsSortedVector<index_t> >& old, const std::vector<gsSortedVector<index_t> >& n, const gsSparseMatrix<T,RowMajor> & transfer) const
// {
...
// template<short_t d, class T>
// gsSparseMatrix<T> gsDeprecated<d,T>::coarsening_direct( const std::vector<gsSortedVector<index_t> >& old, const std::vector<gsSortedVector<index_t> >& n, const std::vector<gsSparseMatrix<T,RowMajor> >& transfer) const
// {
//     int size1= 0;int size2 = 0;
//     int glob_numb = 0;//continous numbering of hierarchical basis
//     for(unsigned int i =0; i< old.size();i++){//count the number of basis functions in old basis
...
// template<short_t d, class T>
// gsSparseMatrix<T> gsDeprecated<d,T>::coarsening_direct2( const std::vector<gsSortedVector<index_t> >& old,
...
// template<short_t d, class T>
// class gsXml< gsDeprecated<d,T> >
// {
// private:
//     gsXml() { }
// public:
//     GSXML_COMMON_FUNCTIONS(gsDeprecated<TMPLA2(d,T)>);
//     static std::string tag () { return "Basis"; }
//     static std::string type () { return "HBSplineBasis"+ (d>1 ? to_string(d):""); }
    
//     static gsDeprecated<d,T> * get (gsXmlNode * node)
//     {
//         return getHTensorBasisFromXml< gsDeprecated<d,T> > (node);
//     }
  
//     static gsXmlNode * put (const gsDeprecated<d,T> & obj,
//                             gsXmlTree & data )
//     {
//         return putHTensorBasisToXml< gsDeprecated<d,T> > (obj, data);
//     }
// };
