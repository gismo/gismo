...
*/
    ...
    void createArgyrisSpace()
    {
        ...
        gsInfo << "size 1:" << vertex_bf.size() << "\n";
        gsInfo << "size 2:" << vertex_bf[0].size() << "\n";
        ...
        for (size_t numVer = 0; numVer < m_mp.vertices().size(); numVer++)
        {
            ...
            gsWriteParaview(vertex_bf[0][2],"testtesttest",2000);
            gsWriteParaview(vertex_bf[0][3],"testtesttest2",2000);
            if (patchIndex.size() == 3 && m_optionList.getSwitch("noVertex"))
            {
                gsMultiPatch<T> mp_vertex;
                for(size_t i = 0; i < patchIndex.size(); i++)
                    mp_vertex.addPatch(m_mp.patch(patchIndex[i]));
                mp_vertex.computeTopology();

                gsMatrix<T> coeffs_mat;
                coeffs_mat.setZero(mp_vertex.nPatches()*4, mp_vertex.nInterfaces()*5);
/*
                for (size_t numInt = 0; numInt < mp_vertex.nInterfaces(); numInt++)
                {
                    const boundaryInterface item = mp_vertex.interfaces()[numInt];
                    for (index_t patch_int = 0; patch_int < 2; patch_int++) {
                        index_t i = patch_int == 0 ? item.first().patch : item.second().patch;
                        gsInfo << patchIndex[i] << " : " << vertIndex[i] << "\n";

                        gsMatrix<> points(2, 1);
                        points.setZero();
                        points(0, 0) = vertIndex[i] % 2 == 1 ? 0 : 1;
                        points(1, 0) = vertIndex[i] < 3 ? 0 : 1;

                        gsInfo << "Patches: " << vertex_bf[patchIndex[i]][vertIndex[i] - 1].nPatches() << "\n";

                        for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][vertIndex[i] - 1].nPatches(); ii++) {
                            gsMatrix<> basisVal = vertex_bf[patchIndex[i]][vertIndex[i] - 1].patch(ii).eval(points);
                            gsMatrix<> basisDer = vertex_bf[patchIndex[i]][vertIndex[i] - 1].patch(ii).deriv(points);
                            gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][vertIndex[i] - 1].patch(ii).deriv2(points);
                            coeffs_mat(i * 4 + 0, numInt * 5 + ii) = basisVal(0, 0);
                            coeffs_mat(i * 4 + 1, numInt * 5 + ii) = basisDer(0, 0); // u
                            coeffs_mat(i * 4 + 2, numInt * 5 + ii) = basisDer(1, 0); // v
                            coeffs_mat(i * 4 + 3, numInt * 5 + ii) = basisDer2(2, 0); // uv
                        }
                    }
                }
*/
                ...
                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][3].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][3].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][3].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][3].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, 10 + ii) = -basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, 10 + ii) = -basisDer(0, 0); // u
                    if (basisDer(1, 0) * basisDer(1, 0) > 1e-25)
                        coeffs_mat(i * 4 + 2, 10 + ii) = -basisDer(1, 0); // v
                    if (basisDer2(2, 0) * basisDer2(2, 0) > 1e-25)
                        coeffs_mat(i * 4 + 3, 10 + ii) = -basisDer2(2, 0); // uv
                }

                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][7].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][7].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][7].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][7].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, ii) = basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, ii) = basisDer(0, 0); // u
                    if (basisDer(1, 0) * basisDer(1, 0) > 1e-25)
                        coeffs_mat(i * 4 + 2, ii) = basisDer(1, 0); // v
                    if (basisDer2(2, 0) * basisDer2(2, 0) > 1e-25)
                        coeffs_mat(i * 4 + 3, ii) = basisDer2(2, 0); // uv
                }
                ...
                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][1].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][1].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][1].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][1].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, 10+ii) = basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, 10+ii) = basisDer(0, 0); // u
                ...
                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][6].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][6].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][6].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][6].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, 5+ii) = -basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, 5+ii) = -basisDer(0, 0); // u
                    if (basisDer(1, 0) * basisDer(1, 0) > 1e-25)
                        coeffs_mat(i * 4 + 2, 5+ii) = -basisDer(1, 0); // v
                    if (basisDer2(2, 0) * basisDer2(2, 0) > 1e-25)
                        coeffs_mat(i * 4 + 3, 5+ii) = -basisDer2(2, 0); // uv
                }
                ...
                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][0].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][0].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][0].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][0].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, ii) = -basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, ii) = -basisDer(0, 0); // u
                ...
                for (size_t ii = 0; ii < vertex_bf[patchIndex[i]][4].nPatches(); ii++) {
                    gsMatrix<> basisVal = vertex_bf[patchIndex[i]][4].patch(ii).eval(points);
                    gsMatrix<> basisDer = vertex_bf[patchIndex[i]][4].patch(ii).deriv(points);
                    gsMatrix<> basisDer2 = vertex_bf[patchIndex[i]][4].patch(ii).deriv2(points);
                    if (basisVal(0, 0) * basisVal(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 0, 5+ii) = basisVal(0, 0);
                    if (basisDer(0, 0) * basisDer(0, 0) > 1e-25)
                        coeffs_mat(i * 4 + 1, 5+ii) = basisDer(0, 0); // u
                ...
                gsMultiPatch<> temp_result;
                temp_result.addPatch(vertex_bf[patchIndex[0]][3].patch(0));
                temp_result.addPatch(vertex_bf[patchIndex[0]][3].patch(0));
                temp_result.addPatch(vertex_bf[patchIndex[0]][3].patch(0));

                gsMatrix<> coef_bf0, coef_bf0_temp, coef_bf1, coef_bf2;
                coef_bf0.setZero(vertex_bf[patchIndex[0]][3].patch(0).coefs().rows(), 1);
                coef_bf1.setZero(vertex_bf[patchIndex[0]][3].patch(0).coefs().rows(), 1);
                coef_bf2.setZero(vertex_bf[patchIndex[0]][3].patch(0).coefs().rows(), 1);

                for (size_t ii = 0; ii < vertex_bf[patchIndex[1]][4].nPatches(); ii++) {

                    i = 0;
                    //coef_bf0 += vertex_bf[patchIndex[i]][3].patch(ii).coefs() * kernel(i * 5 + ii, 0);
                    coef_bf0 += vertex_bf[patchIndex[i]][7].patch(ii).coefs() * kernel(i * 5 + ii, 0);

                    i = 2;
                    gsInfo << kernel(i * 5 + ii, 0) << "\n";
                    coef_bf2 += vertex_bf[patchIndex[i]][6].patch(ii).coefs();// * kernel(i * 5 + ii, 0);
                    //coef_bf2 += vertex_bf[patchIndex[i]][1].patch(ii).coefs() * kernel(i * 5 + ii, 0);

                    i = 1;
                    coef_bf1 += vertex_bf[patchIndex[i]][0].patch(ii).coefs() * kernel(i * 5 + ii, 0);
                    coef_bf1 += vertex_bf[patchIndex[i]][4].patch(ii).coefs() * kernel(i * 5 + ii, 0);
                }

                coef_bf0_temp.setZero(vertex_bf[patchIndex[0]][3].patch(0).coefs().rows(), 1);
                coef_bf0_temp(vertex_bf[patchIndex[0]][3].patch(0).coefs().rows()-1,0) = 1;
                //coef_bf0 -= coef_bf0_temp;
                gsInfo << coef_bf0 << "\n";
                gsInfo << vertex_bf[patchIndex[0]][7].patch(0).coefs() + vertex_bf[patchIndex[0]][7].patch(1).coefs() << "\n";

                temp_result.patch(0).setCoefs(coef_bf0);

                gsField<> temp_field_wrong(m_mp.patch(2), vertex_bf[patchIndex[2]][6].patch(0));
                gsWriteParaview(temp_field_wrong, "wrong1",2000);
                gsField<> temp_field_wrong2(m_mp.patch(2), vertex_bf[patchIndex[2]][6].patch(1));
                gsWriteParaview(temp_field_wrong2, "wrong2",2000);
                gsField<> temp_field_wrong3(m_mp.patch(2), vertex_bf[patchIndex[2]][6].patch(2));
                gsWriteParaview(temp_field_wrong3, "wrong3",2000);
                gsField<> temp_field_wrong4(m_mp.patch(2), vertex_bf[patchIndex[2]][6].patch(3));
                gsWriteParaview(temp_field_wrong4, "wrong4",2000);
                gsField<> temp_field_wrong5(m_mp.patch(2), vertex_bf[patchIndex[2]][6].patch(4));
                gsWriteParaview(temp_field_wrong5, "wrong5",2000);
                ...
                points(0, 0) = vertIndex[i] % 2 == 1 ? 0 : 1;
                points(1, 0) = vertIndex[i] < 3 ? 0 : 1;
                gsInfo << "vertex: " << temp_result.patch(0).eval(points) << "\n";

                temp_result.patch(1).setCoefs(coef_bf1);
                temp_result.patch(2).setCoefs(coef_bf2);
                i = 2;
                points.setZero();
                points(0, 0) = vertIndex[i] % 2 == 1 ? 0 : 1;
                points(1, 0) = vertIndex[i] < 3 ? 0 : 1;
                gsInfo << "vertex 2: " << temp_result.patch(2).eval(points) << "\n";

                gsField<> temp_field(m_mp.patch(0), temp_result.patch(0));
                gsField<> temp_field1(m_mp.patch(1), temp_result.patch(1));
                gsField<> temp_field2(m_mp.patch(2), temp_result.patch(2));

                gsWriteParaview(temp_field, "please0", 2000);
                gsWriteParaview(temp_field1, "please1", 2000);
                gsWriteParaview(temp_field2, "please2", 2000);
