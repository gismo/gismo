...
/// A halfedge data structure for polygonal meshes: topology only.
///
/// This class carries everything about a polygonal mesh that does not depend
/// on the coordinate type: the Vertex/Halfedge/Edge/Face handles, their
/// connectivity, the property containers, the iterators and circulators, and
/// all purely combinatorial operations.  Vertex positions and every operation
/// needing them live in the derived gsSurfMesh<Scalar>.
///
/// Being non-templated, all of this compiles exactly once into the library
/// instead of once per scalar type.
///
/// \sa gsSurfMesh
class GISMO_EXPORT gsSurfMeshTopology
{
public:
...
public:
    ...
    gsSurfMeshTopology(const gsSurfMeshTopology& rhs) { operator=(rhs); }

    /// assign \c rhs to \c *this. performs a deep copy of all properties.
    gsSurfMeshTopology& operator=(const gsSurfMeshTopology& rhs);

    /// move \c rhs to \c *this
    gsSurfMeshTopology& operator=(gsSurfMeshTopology&& rhs) noexcept;

    /// assign \c rhs to \c *this. does not copy custom properties.
    gsSurfMeshTopology& assign(const gsSurfMeshTopology& rhs);
...
public:
    ...
    inline Halfedge forward_halfedge(gsSurfMeshTopology::Halfedge he, int r) const
    {
        for (int k = 0; k < r; k++)
    ...
    inline Halfedge backward_halfedge(gsSurfMeshTopology::Halfedge he, int r) const
    {
        for (int k = 0; k < r; k++)
    ...
    inline Halfedge next_halfedge(gsSurfMeshTopology::Halfedge he, int r) const
    {
        for (int k = 0; k < r; k++)
            he = next_halfedge(he);
        return he;
    ...
    inline Halfedge prev_halfedge(gsSurfMeshTopology::Halfedge he, int r) const
    {
        for (int k = 0; k < r; k++)
            he = prev_halfedge(he);
        return he;
...
inline std::ostream& operator<<(std::ostream& os, gsSurfMeshTopology::Vertex v)
{ return (os << 'v' << v.idx()); }

/// print a halfedge handle
inline std::ostream& operator<<(std::ostream& os, gsSurfMeshTopology::Halfedge h)
{ return (os << 'h' << h.idx()); }

/// print an edge handle
inline std::ostream& operator<<(std::ostream& os, gsSurfMeshTopology::Edge e)
{ return (os << 'e' << e.idx()); }

/// print a face handle
inline std::ostream& operator<<(std::ostream& os, gsSurfMeshTopology::Face f)
{ return (os << 'f' << f.idx()); }
