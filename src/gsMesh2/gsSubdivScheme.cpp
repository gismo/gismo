...
namespace gismo {
    ...
    using MeshT = gsSurfMesh<real_t>;
    typedef MeshT::Point Point;
    typedef MeshT::Vertex Vertex;
    typedef MeshT::Face Face;
    typedef MeshT::Halfedge Halfedge;
    typedef MeshT::Edge Edge;
    ...
    MeshT::Vertex_property<Point> gsSubdivScheme::vertex_limits()
    {
        const index_t s = m_options.getInt("scheme");
        switch (s)
        {
    ...
    MeshT::Vertex_property<Point> gsSubdivScheme::tangent_vertex_limits()
    {
        const index_t s = m_options.getInt("scheme");
        switch (s)
        {
    ...
    MeshT::Vertex_property<Point> gsSubdivScheme::normals_vertex_limits()
    {
        const index_t s = m_options.getInt("scheme");
        switch (s)
        {
    ...
    void gsSubdivScheme::cc_subdivide()
    {
        ...
        MeshT::Halfedge_property<bool> sharp;
        if (do_sharp)
        ...
        for (auto fit : m_mesh->faces())
        {
            v = MeshT::Vertex(fnv + (i++));//face vertex id ?
...
#   pragma omp parallel for default(shared) private(v)
        for (i = 0; i < env; ++i)
        {
            v = MeshT::Vertex(i); // original vertices
...
#   pragma omp parallel for default(shared) private(n)
    ...
    MeshT::Vertex_property<MeshT::Point>
