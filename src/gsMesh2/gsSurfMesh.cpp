...
void gsSurfMesh::cc_subdivide()
{
    ...
    {
        he  = halfedge(eit,0);
        if ( !(is_boundary(he) && is_boundary(opposite_halfedge(he))) )
        {
            tmp = (vpoint_[from_vertex(he)]+vpoint_[to_vertex(he)]) / 2;
    ...
    for (i = 0; i<env;++i)
    {
        ...
        if (do_sharp && has_flag(v,sharp))
        {
            ...
            if (2==sd)
            {
                ...
                pt *= 2; //2~6
                pt += vpoint_[to_vertex(h2)]; // first flagged neighbor
                pt += vpoint_[to_vertex(h1)]; // second flagged neighbor
                pt /= 4; // /4~8
            }
        ...
        if (is_boundary(v))
        {
            ...
            pt += vpoint_[to_vertex(hh)]; //  right boundary neighbor
            hh = next_halfedge(opposite_halfedge(hh));
            pt += vpoint_[to_vertex(hh)]; // left boundary neighbor
            pt /= 4;
        }
        else
        {
            ...
            if (vit)
                do
                { //pt += 4*E-F
                    pt += 4*vpoint_[ to_vertex(*vit) ]
                        - vpoint_[ to_vertex(next_halfedge(*vit)) ];
                } while (++vit != vcp);
            pt /= n*n;
        }
    }
...
gsSurfMesh::ds_image_point_calc(Vertex oldv, Face oldf)
{
    ...
    if (is_boundary(oldv)) {
        ...
        if (valence(oldv) == 2) { // Corner's case (same position because it will go to limit)
            coords = position(oldv);
        }
        else { // Chaikin method for the boundary
            if (is_boundary(to_vertex(hf))) {
                coords = 0.75 * position(oldv) + 0.25 * position(to_vertex(hf));
            }
            else {
                coords = 0.25 * position(from_vertex(prev_halfedge(hf))) + 0.75 * position(oldv);
            }
        }
