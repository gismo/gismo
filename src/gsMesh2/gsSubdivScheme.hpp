/** @file gsSubdivScheme.hpp

    @brief Subdivision operations on mesh (implementation).
...
void gsSubdivScheme<Scalar>::subdivide(index_t numSubs)
{
...
gsSubdivScheme<Scalar>::vertex_limits()
{
    const index_t s = m_options.getInt("scheme");
    switch (s)
    {
...
template <class Scalar>
typename gsSurfMesh<Scalar>::template Vertex_property<typename gsSurfMesh<Scalar>::Point> 
gsSubdivScheme<Scalar>::tangent_vertex_limits()
{
    const index_t s = m_options.getInt("scheme");
    switch (s)
    {
...
template <class Scalar>
typename gsSurfMesh<Scalar>::template Vertex_property<typename gsSurfMesh<Scalar>::Point> 
gsSubdivScheme<Scalar>::normals_vertex_limits()
{
    const index_t s = m_options.getInt("scheme");
    switch (s)
    {
...
template <class Scalar>
void gsSubdivScheme<Scalar>::cc_subdivide()
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
    typedef typename gsSurfMesh<Scalar>::Halfedge MeshHalfedge;
    ...
    if (do_sharp)
        sharp = m_mesh->template get_halfedge_property<bool>("h:sharp");
    ...
    for (auto fit : m_mesh->faces())
    {
        v = MeshVertex(fnv + (i++));//face vertex id ?
...
#   pragma omp parallel for default(shared) private(v)
    for (i = 0; i < env; ++i)
    {
        v = MeshVertex(i); // original vertices
...
template <class Scalar>
typename gsSurfMesh<Scalar>::template Vertex_property<typename gsSurfMesh<Scalar>::Point>
gsSubdivScheme<Scalar>::cc_vertex_limits(std::string label)
{
    typedef typename gsSurfMesh<Scalar>::Point Point;
    
    auto points = m_mesh->template get_vertex_property<Point>("v:point");
    auto limits = m_mesh->template add_vertex_property<Point>(
...
template <class Scalar>
typename gsSurfMesh<Scalar>::template Vertex_property<typename gsSurfMesh<Scalar>::Point>
gsSubdivScheme<Scalar>::cc_normals_vertex_limits(std::string label, bool normalize)
{
    typedef typename gsSurfMesh<Scalar>::Point Point;
    typedef typename gsSurfMesh<Scalar>::Halfedge MeshHalfedge;
    ...
    auto limits = m_mesh->template add_vertex_property<Point>(label, Point(0, 0, 0));
    Point t1, t2;
...
#   pragma omp parallel for default(shared) private(h2,t1,t2,c1,c2,cc1,cc2,i)
    for (auto vit = m_mesh->vertices_begin(); vit < m_mesh->vertices_end(); ++vit)
    {
        const Scalar n = m_mesh->valence(*vit);
...
template <class Scalar>
typename gsSurfMesh<Scalar>::template Vertex_property<typename gsSurfMesh<Scalar>::Point>
gsSubdivScheme<Scalar>::cc_tangent_vertex_limits(std::string label, bool normalize)
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
    typedef typename gsSurfMesh<Scalar>::Halfedge MeshHalfedge;
    typedef typename gsSurfMesh<Scalar>::Point Point;
...
template <class Scalar>
void gsSubdivScheme<Scalar>::ds_subdivide()
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
    typedef typename gsSurfMesh<Scalar>::Face MeshFace;
    typedef typename gsSurfMesh<Scalar>::Point Point;
    ...
    std::map<std::pair<MeshVertex, MeshFace>, MeshVertex> Map;


    // Create of V-Faces
    MeshVertex v;
    std::vector<MeshVertex> ffv;
...
gsSubdivScheme<Scalar>::ds_image_point_calc_interpolation(
    typename gsSurfMesh<Scalar>::Vertex oldv, 
    typename gsSurfMesh<Scalar>::Face oldf)
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
    typedef typename gsSurfMesh<Scalar>::Halfedge MeshHalfedge;
    typedef typename gsSurfMesh<Scalar>::Point Point;
    ...
    MeshHalfedge hf = m_mesh->halfedge(oldf);
    ...
    typename gsSurfMesh<Scalar>::Point coords;
    coords.setZero();
    ...
    if (m_mesh->is_boundary(oldv)) {
        ...
        Scalar sum_val{ val };
        MeshHalfedge next_he{ m_mesh->next_halfedge(hf) };
...
template <class Scalar>
typename gsSurfMesh<Scalar>::Point
gsSubdivScheme<Scalar>::ds_image_point_calc_vanila(
    typename gsSurfMesh<Scalar>::Vertex oldv, 
    typename gsSurfMesh<Scalar>::Face oldf)
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
...
template <class Scalar>
void gsSubdivScheme<Scalar>::loop_subdivide()
{
    typedef typename gsSurfMesh<Scalar>::Vertex MeshVertex;
    typedef typename gsSurfMesh<Scalar>::Halfedge MeshHalfedge;
    typedef typename gsSurfMesh<Scalar>::Edge MeshEdge;
    typedef typename gsSurfMesh<Scalar>::Face MeshFace;
    typedef typename gsSurfMesh<Scalar>::Point Point;
    ...
    MeshHalfedge he,nh, ph, hh;
    Point tmp;
    std::map<MeshEdge, MeshVertex> edgeVerts;
    std::map<MeshVertex, MeshVertex> innerVerts;
    ...
    for (auto ho : m_mesh->halfedges()) 
    {
        ...
        {
            tmp = (Scalar)1 / 2 * m_mesh->position(m_mesh->from_vertex(ho)) +
                (Scalar)1 / 2 * m_mesh->position(m_mesh->to_vertex(ho));
        }
        ...
        else 
        {
            ...
            tmp = (Scalar)3 / 8 * m_mesh->position(m_mesh->from_vertex(ho)) +
                (Scalar)3 / 8 * m_mesh->position(m_mesh->to_vertex(ho)) +
                (Scalar)1 / 8 * m_mesh->position(m_mesh->to_vertex(nh)) +
                (Scalar)1 / 8 * m_mesh->position(m_mesh->from_vertex(ph));
        }
    ...
    for (auto it = 0; it < fnv; it++) 
    {
        ...
        if (m_mesh->is_boundary(v)) 
        {
            ...
            he = hh;
            // Apply boundary mask
            tmp = (Scalar)3 / 4 * m_mesh->position(m_mesh->from_vertex(he)) +
                (Scalar)1 / 8 * m_mesh->position(m_mesh->to_vertex(he));
            hh = he;
            do 
            {
                hh = m_mesh->ccw_rotated_halfedge(hh);
                ...
                {
                    tmp += (Scalar)1 / 8 * m_mesh->position(m_mesh->to_vertex(hh));
                }
        ...
        else 
        {
            ...
            {  // Loop's original mask (1987)
                a = (0.625 - (0.375 + 0.25 * cos(2 * 3.14159 / (Scalar)n)) * (0.375 + 0.25 * cos(2 * 3.14159 / (Scalar)n))) / (Scalar)n;
            }
            b = a;
            c = 1.0 - a * n;
    ...
    MeshEdge e;
    MeshFace f;
    std::vector<MeshVertex> edgeVertsFace;
