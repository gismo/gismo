...
///    V3         E3        V2
///
/// where C0-C3 are the vertices in the order mesh.vertices(face) traverses them
/// and E0-E3 are the (half)edges in the order mesh.halfedges(face) traverses
/// them. In particular, the position of control points 0, 4, 20, 24 should
/// correspond to the position of the vertices (if present).
...
public: // Contructors
    /// Default contstructor with zeroed inner control points and empty face
    /// reference.
    ...
    /// patch to achieve C1 smoothness.
...
public: // Control point accessors
    ...
    /// row or column on this halfedge. I.e. with inset 1, one row/column
    /// farther in and without the first and last element. I.e. with inset 2,
    /// two rows/columns farther in and without the first, second,
    /// second-to-last and last element. If the given half edge does not belong
    /// to this face, this will fail in debug mode and return an empty vector.
    /// If the inset is to large (greater than half the size `N` of the control
    /// net), this will return an empty vector. Pointers returned by this
    /// function are valid as long as the underlying mesh remains unchanged and
    /// should be regenerated after.
    ...
    /// the correct corner of the control net associated to this vertex. I.e.
