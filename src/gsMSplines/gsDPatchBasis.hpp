...
namespace gismo
{
    ...
    gsDPatchBasis<d,T>::gsDPatchBasis(const gsMultiPatch<T> & patches)
    :
    m_patches(patches)
    {
        m_verbose = false;
        ...
        m_multiBasis = gsMultiBasis<T>(m_patches);

        m_mapModified = gsDofMapper(m_multiBasis);
        m_mapOriginal = gsDofMapper(m_multiBasis);

        m_size = -1;

        this->initialize();
        this->initializeMapper();
        this->computeSmoothMatrix2();
        this->makeTHB();
        this->computeDPatch();
    }

    template<short_t d,class T>
    gsDPatchBasis<d,T>::gsDPatchBasis(const gsDPatchBasis& other)
    :
    ...
    //     for(ConstBasisIter it = other.m_multiBasis.begin();it!=other.m_multiBasis.end();++it)
    //     {
    //         m_multiBasis.push_back( (BasisType*)(*it)->clone().release() );
    //     }
    ...
    template<short_t d,class T>
    gsDPatchBasis<d,T>::~gsDPatchBasis()
    {
        freeAll(m_multiBasis);
    }
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsMultiBasis<T> gsDPatchBasis<d,T>::makeGlobalBasis()
    {
        m_MBasis = gsMappedBasis<d,T>(m_multiBasis,m_matrix.transpose());

        m_basisContainer = std::vector<gsBasis<T> *>(m_patches.nPatches());
        for (size_t p=0; p!=m_patches.nPatches(); p++)
            m_basisContainer[p] = &m_MBasis.getMappedSingleBasis(p);
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::mapperInfo() const
    {
        for (size_t p=0; p!=m_patches.nPatches(); p++)
        {
            gsInfo<<"----------------------------------\n";
    ...
    template<short_t d,class T>
    const void gsDPatchBasis<d,T>::vertexInfo(patchCorner corner) const
    {
    ...
    template<short_t d,class T>
    const void gsDPatchBasis<d,T>::sideInfo(patchSide side) const
    {
        gsInfo<<"Patch "<<side.patch<<", side "<<side<<" is "<<(m_patches.isBoundary(side) ? "a boundary side" : "an interface")<<"\n";
    }

    template<short_t d,class T>
    const void gsDPatchBasis<d,T>::sideInfo() const
    {
    ...
    template<short_t d,class T>
    const void gsDPatchBasis<d,T>::cornerInfo() const
    {
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsMatrix<T> gsDPatchBasis<d,T>::preCoefficients()
    {
                ...
                if (vdata.first==3 && !vdata.second)
                {
                    ...
                    for (std::vector<patchCorner>::iterator corner = otherCorners.begin(); corner != otherCorners.end(); corner++)
                    {
                        ...
                        if ( m_patches.isBoundary(csides[0]) || m_patches.isBoundary(csides[1]) ) //
                            b11b.push_back(m_mapModified.index( indexFromVert(m_multiBasis0,1,*corner,csides[0],1) , corner->patch) );
                        else if (b11i==-1)
                            b11i = m_mapModified.index( indexFromVert(m_multiBasis0,1,*corner,csides[0],1) , corner->patch);
                        else
                            GISMO_ERROR("b11i is already assigned?");

                        if (corner==otherCorners.begin())
                        {
                            gsBasis <> * basis = &m_multiBasis0.basis(corner->patch);
                            b00 = m_mapModified.index( basis->functionAtCorner(corner->corner()), corner->patch );
                        }

                        idx = vertIndex(corner->patch,corner->corner());
    ...
    template<short_t d,class T>
    gsMatrix<T> gsDPatchBasis<d,T>::allCoefficients() const
    {
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsGeometry<T>* gsDPatchBasis<d,T>::exportPatch(index_t patch, bool computeCoefs)
    {
        ////////////////////////////////////////////////
        ...
        return m_multiBasis[patch].makeGeometry( give(local) ).release();
    }

    template<short_t d,class T>
    gsMultiPatch<T> gsDPatchBasis<d,T>::exportToPatches()
    {
        m_coefs = this->preCoefficients();
        m_coefs = m_matrix.transpose() * m_coefs;
    ...
    template<short_t d,class T>
    const index_t gsDPatchBasis<d,T>::indexFromSides(index_t index1, const patchSide side1, index_t index2, const patchSide side2)
    {
        /*
        ...
        */
        ...
        gsBasis<T> * basis = &m_multiBasis.basis(side1.patch);
    ...
    template<short_t d,class T>
    const gsVector<index_t> gsDPatchBasis<d,T>::indicesFromVert(index_t index, const patchCorner corner, const patchSide side, index_t offset)
    {
        /*
        ...
        */
        ...
        gsBasis<T> * basis = &m_multiBasis.basis(side.patch);
    ...
    template<short_t d,class T>
    const index_t gsDPatchBasis<d,T>::indexFromVert(index_t index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        return indexFromVert(m_multiBasis,index, corner, side, offset, levelOffset);
    }

    template<short_t d,class T>
    const index_t gsDPatchBasis<d,T>::indexFromVert(gsMultiBasis<T> bases, index_t index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        if ((index==0) && (offset==0))
        {
        ...
        else
        {
            ...
            indices.at(0) = index;
            std::vector<index_t> result = indexFromVert(bases,indices,corner,side,offset,levelOffset);
            return result.at(0);
        }
    }

    template<short_t d,class T>
    const std::vector<index_t> gsDPatchBasis<d,T>::indexFromVert(std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        return indexFromVert(m_multiBasis,index,corner,side,offset,levelOffset);
    }

    template<short_t d,class T>
    const std::vector<index_t> gsDPatchBasis<d,T>::indexFromVert(gsMultiBasis<T> bases, std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        /*
    ...
    template<short_t d,class T>
    const std::pair<index_t,bool> gsDPatchBasis<d,T>::vertexData(const patchCorner corner) const
    {
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::makeTHB()
    {
        ...
        if (cornerLists.size()!=0)
        {
            ...
            m_multiBasis = gsMultiBasis<T>(m_RefPatches);
        }
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::computeDPatch()
    {
        /*
                ...
                for (size_t v =0; v!=cornerLists.size(); v++) // over EVs
                {
                    ...
                    */
                    ...
                    for (index_t i = 0; i!=N; i++) // over corners in EVs
                    {
                        ...
                        c11(patches[side.patch],0) = indexFromVert(1,corners[i],side,1,0);
                        c12(patches[side.patch],0) = indexFromVert(2,corners[i],side,1,0);
                        c21(patches[otherSide.patch],0) = indexFromVert(2,otherCorner,otherSide,1,0);

                        c11o(patches[side.patch],0) = indexFromVert(1,corners[i],side,1,-1);
                        c12o(patches[side.patch],0) = indexFromVert(2,corners[i],side,1,-1);
                        c21o(patches[otherSide.patch],0) = indexFromVert(2,otherCorner,otherSide,1,-1);
                    ...
                    for (index_t i = 0; i!=N; i++)
                    {
                        ...
                        // rowIndices(i,0) = m_mapModified.index(indexFromVert(1,corner,sides[0],1,-1),corner.patch);
                    ...
                    for (index_t i = 0; i!=N; i++) // for all involved corners
                    {
                        for (index_t k=0; k!=3; k++)
                        {
                            for (index_t j=0; j!=N; j++) // loop over the connected corners
                            {
                                ...
                                colIdx = indexFromVert(0,corners[j],sides[0],0,0); // 0,0
                                ...
                                colIdx = indexFromVert(1,corners[j],sides[0],0,0); // 1,0
                                colIdx = m_mapOriginal.index(colIdx,corners[j].patch);
                                m_matrix(rowIndices(i+k*N,0),colIdx) = m_matrix(rowIndices(i+k*N,0),c11(i,0)); // by construction

                                colIdx = indexFromVert(1,corners[j],sides[1],0,0); // 0,1
                                colIdx = m_mapOriginal.index(colIdx,corners[j].patch);
                                m_matrix(rowIndices(i+k*N,0),colIdx) = m_matrix(rowIndices(i+k*N,0),c11(i,0)); // by construction
                            }
                        }
                    ...
                    for (index_t i = 0; i!=N; i++) // for all involved interfaces
                    {
                        for (index_t k = 2; k!=4 ; k++)// std::max(basis1->maxDegree(),basis2->maxDegree())+
                        {
                            ...
                            idx = indexFromVert(k,corner,side,0,0);
                            index_t j0k = m_mapOriginal.index(idx,side.patch);

                            idx = indexFromVert(k,otherCorner,otherSide,0,0);
                            index_t jk0 = m_mapOriginal.index(idx,otherSide.patch);

                            idx = indexFromVert(k,corner,side,1,0);         // point (k,0)
                            index_t jk1 = m_mapOriginal.index(idx,side.patch); // point (k,0)

                            idx = indexFromVert(k,otherCorner,otherSide,1,0);         // point (k,0)
    ...
    template<short_t d,class T>
    gsMatrix<T> gsDPatchBasis<d,T>::makePi(index_t valence)
    {
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::initialize() // also initialize the mappers!
    {
        // Cast all patches of the mp object to THB splines
        gsTHBSpline<d,T> thb;
        for (size_t k=0; k!=m_patches.nPatches(); ++k)
        {
            if (typeid(gsTHBSplineBasis<2,T>)!=typeid(m_multiBasis.basis(k)))
            {
        ...
        for (size_t k=0; k!=m_patches.nPatches(); k++)
            tmp += (m_multiBasis.basis(k).component(0).size()-4)*(m_multiBasis.basis(k).component(1).size()-4);
        ...
        {
            basis1 = &m_multiBasis.basis(iit->first().patch);
            basis2 = &m_multiBasis.basis(iit->second().patch);
        ...
        {
            basis1 = &m_multiBasis.basis(bit->patch);
        ...
        m_size += tmp;
        // redefine the ot
        m_multiBasis0 = m_multiBasis = gsMultiBasis<T>(m_patches);

        m_mapModified = gsDofMapper(m_multiBasis);
        m_mapOriginal = gsDofMapper(m_multiBasis);

        // gsWriteParaview(m_patches,"mp",1000,true);
        // gsWriteParaview<>( m_multiBasis.basis(0), "basis", 1000, true);

        m_matrix.resize(m_size,m_multiBasis.totalSize());
        // m_matrix.resize(m_multiBasis.totalSize(),m_multiBasis.totalSize());

        m_coefs = this->allCoefficients();
    }

    template<short_t d,class T>
    void gsDPatchBasis<d,T>::initializeMapper() // also initialize the mappers!
    {
        // interfaces
        std::fill(m_sideCheck.begin(), m_sideCheck.end(), false);
        std::fill(m_vertCheck.begin(), m_vertCheck.end(), false);
        ...
        for(gsBoxTopology::const_iiterator iit = m_patches.iBegin(); iit!= m_patches.iEnd(); iit++)
        {
            ...
            for (index_t p = 0; p != 2; p++)
            {
                ...
                */
                basis = &m_multiBasis.basis(patches[p]);
                ...
                {
                    selectedIndices.push_back(indexFromVert(0,pcorners[c],psides[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                    selectedIndices.push_back(indexFromVert(1,pcorners[c],psides[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                }
                ...
                // std::cout << "The difference has " << (result.size()) << " elements:\n";
        ...
        for (size_t p=0; p!=m_patches.nPatches(); p++)
        {
            for (index_t c=1; c<=4; c++)
            {
                ...
                basis = &m_multiBasis.basis(pcorner.patch);
                        ...
                        */
                        ...
                        for (size_t c=0; c!=pcorners.size(); c++)
                        {
                            ...
                            if (c!=0)
                            {
                                ...
                                // m_mapModified.eliminateDof(indexFromVert(0,pcorners[c],pseudo),pcorners[c].patch);
                                ...
                                // gsDebug<<"ELIMINATED "<<indexFromVert(0,pcorners[c],pseudo)<<" of basis "<<pcorners[c].patch<<"\n";
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::computeSmoothMatrix()
    {
        m_basisCheck.resize(m_size);
        // m_basisCheck.resize(m_multiBasis.totalSize());
        ...
        m_matrix.resize( m_mapModified.freeSize(), m_multiBasis.totalSize() );
        // m_matrix.resize( m_multiBasis.totalSize(), m_multiBasis.totalSize() );
        ...
        for(gsBoxTopology::const_iiterator iit = m_patches.iBegin(); iit!= m_patches.iEnd(); iit++)
        {
            ...
            basis[0] = &m_multiBasis.basis(iit->first().patch);
            basis[1] = &m_multiBasis.basis(iit->second().patch);
                    ...
                    if (!vdata.second) // boundary vertex
                    {
                        ...
                        else if (vdata.first==3) //valence = 3
                        {
                            ...
                            */
                            ...
                            index_t b01_p1 = indexFromVert(1,pcorners[c],csides[0],0); // index from vertex pcorners[c] along side psides[0] with offset 0.
                            index_t b10_p1 = indexFromVert(1,pcorners[c],csides[1],0); // index from vertex pcorners[c] along side psides[0] with offset 0.
                            ...
                            {
                                tmpBasis = &m_multiBasis.basis(otherCorners[k].patch);
                            ...
                            tmpBasis = &m_multiBasis.basis(pcorn.patch);
                            ...
                            // tmpBasis = &m_multiBasis.basis(pcorners[c].patch);
                            ...
                            for (std::vector<patchCorner>::iterator corner = otherCorners.begin(); corner != otherCorners.end(); corner++)
                            {
                                tmpBasis = &m_multiBasis.basis(corner->patch);
            ...
            */
            ...
            for (index_t p =0; p!=2; p++)
            {
                ...
                for (index_t c =0; c!=2; c++)
                {
                    selectedIndices[p].push_back(indexFromVert(0,pcorners[c],psides[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                    selectedIndices[p].push_back(indexFromVert(1,pcorners[c],psides[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.

                    selectedOIndices[p].push_back(indexFromVert(0,pcorners[c],psides[p],1,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                    selectedOIndices[p].push_back(indexFromVert(1,pcorners[c],psides[p],1,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                }
        ...
        for(size_t p=0; p!=m_patches.nPatches(); p++)
            for (index_t b=0; b!=m_multiBasis.basis(p).size(); b++)
            {
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::handleVertex(patchCorner pcorner)
    {

        if (m_vertCheck[ vertIndex(pcorner.patch, pcorner.corner()) ])
        {
        ...
        if (!vdata.second) // boundary vertices
        {
            ...
            if (vdata.first==1)
            {
                ...
                indices.resize(4);
                indices[0] = indexFromVert(0,pcorner,psides[0],0); // b00
                indices[1] = indexFromVert(1,pcorner,psides[0],0); // b01
                indices[2] = indexFromVert(1,pcorner,psides[1],0); // b10
                indices[3] = indexFromVert(1,pcorner,psides[1],1); // b11
                ...
                m_vertCheck[ vertIndex(pcorner.patch, pcorner.corner()) ] = true;
            }
            // Correct
            else if (vdata.first==2)
            {
                ...
                for (index_t k = 0; k!=2; k++) // index of point over the interface
                {
                    ...
                    indices[0] = indexFromVert(k,pcorner,psides[iindex],1); // bk1 on patch of iface
                    indices[1] = indexFromVert(k,pcorner,psides[iindex],0); // bk0 on patch of iface
                    indices[2] = indexFromVert(k,otherCorner,otherSide,0); // bk0 on other patch
            ...
            else if (vdata.first==3)
            {
                ...
                extraRow = indexFromVert(0,lowest,psides[0],0);
                ...
                for (std::vector<patchCorner>::iterator it = corners.begin(); it!=corners.end(); ++it)
                {
                    ...
                    colIndices.push_back( indexFromVert(0,*it,psides[0],0) );
                    rowIndices.push_back( indexFromVert(1,*it,psides[0],1) );
                    ...
                    for (index_t k = 0; k!=2; k++) // index of point over the interface
                    {
                        ...
                        {
                            //
                            index_t colIdx = m_mapOriginal.index(indexFromVert(1,*it,psides[k],0),it->patch);
                            index_t rowIdx = m_mapModified.index(indexFromVert(1,*it,psides[k],1),it->patch);
                            m_matrix(rowIdx,colIdx) = 0.5;
                ...
                for (std::vector<boundaryInterface>::iterator it = ifaces.begin(); it!=ifaces.end(); ++it)
                {
                    ...
                    for (std::vector<patchCorner>::iterator corn = temp_corners.begin(); corn!=temp_corners.end(); ++corn)
                    {
                        ...
                        {
                            rowIndices[k] = indexFromVert(1,icorners[k],isides[k],1);
                            colIndices[k] = indexFromVert(1,icorners[k],isides[k],0);
                ...
                for (std::vector<patchSide>::iterator it = boundaries.begin(); it!=boundaries.end(); ++it)
                {
                    ...
                    for (std::vector<patchCorner>::iterator corn = temp_corners.begin(); corn!=temp_corners.end(); ++corn)
                    {
                        ...
                        index_t colIdx = indexFromVert(1,*corn,*it,0);
                ...
                for (std::vector<patchCorner>::iterator it = corners.begin(); it!=corners.end(); ++it)
                    m_vertCheck[ vertIndex(it->patch, it->corner()) ] = true;
            }
            else
        ...
        else // interior vertices
        {
            ...
            // 1. give the basis function a weight 1 from itself
            index_t b11_p1 = indexFromVert(1,pcorner,psides[0],1); // point 1,1 (does not matter which reference side is taken)
            ...
            for (std::vector<patchSide>::iterator side = psides.begin(); side != psides.end(); ++side)
            {
                ...
                index_t b10_p1 = indexFromVert(1,pcorner,*side,0); // index from vertex pcorners[c] along side psides[0] with offset 0.
                index_t b10_p2 = indexFromVert(1,otherCorner,otherSide,0); // point 0,1
                ...
                {
                    tmpBasis = &m_multiBasis.basis(corn->patch);
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::handleInterface(boundaryInterface iface)
    {
        if (m_sideCheck[ sideIndex(iface.first().patch, iface.first().side()) ] || m_sideCheck[ sideIndex(iface.second().patch, iface.second().side()) ])
        {
        ...
        for (index_t p =0; p!=2; p++)
        {
            np = abs(p-1); // not index p;
            // get the bases belonging to both patches
            basis[p] = &m_multiBasis.basis(iface[p].patch);
            // now we treat the offset of the interface
            ...
            for (index_t c =0; c!=2; c++)
            {
                selectedIndices[p].push_back(indexFromVert(0,pcorners[c],iface[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                selectedIndices[p].push_back(indexFromVert(1,pcorners[c],iface[p],0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.

                selectedOIndices[p].push_back(indexFromVert(0,pcorners[c],iface[p],1,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                selectedOIndices[p].push_back(indexFromVert(1,pcorners[c],iface[p],1,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
            }
        ...
        for (index_t p =0; p!= 2; p++)
        {
            ...
            m_sideCheck[ sideIndex(iface[p].patch, iface[p].side()) ] = true; // side finished
        }

    }

    template<short_t d,class T>
    void gsDPatchBasis<d,T>::handleBoundary(patchSide side)
    {
            std::vector<patchCorner> pcorners;
            std::vector<index_t> selectedIndices;
            gsBasis<T> * basis = &m_multiBasis.basis(side.patch);
            ...
            for (index_t c =0; c!=2; c++)
            {
                selectedIndices.push_back(indexFromVert(0,pcorners[c],side,0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
                selectedIndices.push_back(indexFromVert(1,pcorners[c],side,0,0)); // index from vertex pcorners[c] along side psides[0] with offset 0.
            }
            ...
            m_sideCheck.at( sideIndex(side.patch,side.side()) ) = true;
    }

    template<short_t d,class T>
    void gsDPatchBasis<d,T>::handleInterior()
    {
        index_t rowIdx,colIdx;
        for(size_t p=0; p!=m_patches.nPatches(); p++)
            for (index_t b=0; b!=m_multiBasis.basis(p).size(); b++)
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::whichHandled()
    {
        for (size_t p=0; p!=m_patches.nPatches(); p++)
        {
            gsInfo<<"----------------------------------\n";
    ...
    template<short_t d,class T>
    void gsDPatchBasis<d,T>::computeSmoothMatrix2()
    {
