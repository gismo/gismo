...
namespace gismo
{
    ...
    gsDPatchGeom<d,T>::gsDPatchGeom(const gsMultiPatch<T> & patches)
    :
    m_patches(patches)
    {
        m_verbose = false;
    ...
    template<short_t d,class T>
    gsDPatchGeom<d,T>::gsDPatchGeom(const gsDPatchGeom& other)
    :
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsMultiBasis<T> gsDPatchGeom<d,T>::makeGlobalBasis()
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::mapperInfo() const
    {
        for (size_t p=0; p!=m_patches.nPatches(); p++)
        {
            gsInfo<<"----------------------------------\n";
    ...
    template<short_t d,class T>
    const void gsDPatchGeom<d,T>::vertexInfo(patchCorner corner) const
    {
    ...
    template<short_t d,class T>
    const void gsDPatchGeom<d,T>::sideInfo(patchSide side) const
    {
        gsInfo<<"Patch "<<side.patch<<", side "<<side<<" is "<<(m_patches.isBoundary(side) ? "a boundary side" : "an interface")<<"\n";
    }

    template<short_t d,class T>
    const void gsDPatchGeom<d,T>::sideInfo() const
    {
    ...
    template<short_t d,class T>
    const void gsDPatchGeom<d,T>::cornerInfo() const
    {
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsMatrix<T> gsDPatchGeom<d,T>::preCoefficients()
    {
                ...
                if (vdata.first==3 && !vdata.second)
                {
                    ...
                    for (std::vector<patchCorner>::iterator corner = otherCorners.begin(); corner != otherCorners.end(); corner++)
                    {
                        ...
                        if ( m_patches.isBoundary(csides[0]) || m_patches.isBoundary(csides[1]) ) //
                            b11b.push_back(m_mapModified.index( indexFromVert(m_Bbases,1,*corner,csides[0],1) , corner->patch) );
                        else if (b11i==-1)
                            b11i = m_mapModified.index( indexFromVert(m_Bbases,1,*corner,csides[0],1) , corner->patch);
                        else
    ...
    template<short_t d,class T>
    gsMatrix<T> gsDPatchGeom<d,T>::allCoefficients() const
    {
    ...
    =====================================================================================*/
    template<short_t d,class T>
    gsGeometry<T>* gsDPatchGeom<d,T>::exportPatch(index_t patch, bool computeCoefs)
    {
        ////////////////////////////////////////////////
    ...
    template<short_t d,class T>
    gsMultiPatch<T> gsDPatchGeom<d,T>::exportToPatches()
    {
        m_coefs = this->preCoefficients();
        m_coefs = m_matrix.transpose() * m_coefs;
        ...
        for (size_t p=0; p!=m_patches.nPatches(); p++)
            patches[p]= this->exportPatch(p,true);
    ...
    template<short_t d,class T>
    const index_t gsDPatchGeom<d,T>::indexFromSides(index_t index1, const patchSide side1, index_t index2, const patchSide side2)
    {
        /*
    ...
    template<short_t d,class T>
    const gsVector<index_t> gsDPatchGeom<d,T>::indicesFromVert(index_t index, const patchCorner corner, const patchSide side, index_t offset)
    {
        /*
    ...
    template<short_t d,class T>
    const index_t gsDPatchGeom<d,T>::indexFromVert(index_t index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        return indexFromVert(m_bases,index, corner, side, offset, levelOffset);
    }

    template<short_t d,class T>
    const index_t gsDPatchGeom<d,T>::indexFromVert(gsMultiBasis<T> bases, index_t index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        if ((index==0) && (offset==0))
        {
    ...
    template<short_t d,class T>
    const std::vector<index_t> gsDPatchGeom<d,T>::indexFromVert(std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        return indexFromVert(m_bases,index,corner,side,offset,levelOffset);
    }

    template<short_t d,class T>
    const std::vector<index_t> gsDPatchGeom<d,T>::indexFromVert(gsMultiBasis<T> bases, std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset, index_t levelOffset)
    {
        /*
    ...
    template<short_t d,class T>
    const std::pair<index_t,bool> gsDPatchGeom<d,T>::vertexData(const patchCorner corner) const
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::computeDPatch()
    {
        /*
    ...
    template<short_t d,class T>
    gsMatrix<T> gsDPatchGeom<d,T>::makePi(index_t valence)
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::initialize() // also initialize the mappers!
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::initializeMapper() // also initialize the mappers!
    {
        // interfaces
        std::fill(m_sideCheck.begin(), m_sideCheck.end(), false);
        std::fill(m_vertCheck.begin(), m_vertCheck.end(), false);
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::computeSmoothMatrix()
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::handleVertex(patchCorner pcorner)
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::handleInterface(boundaryInterface iface)
    {
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::handleBoundary(patchSide side)
    {
            std::vector<patchCorner> pcorners;
            std::vector<index_t> selectedIndices;
            gsBasis<T> * basis = &m_bases.basis(side.patch);
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::handleInterior()
    {
        index_t rowIdx,colIdx;
        for(size_t p=0; p!=m_patches.nPatches(); p++)
            for (index_t b=0; b!=m_bases.basis(p).size(); b++)
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::whichHandled()
    {
        for (size_t p=0; p!=m_patches.nPatches(); p++)
        {
            gsInfo<<"----------------------------------\n";
    ...
    template<short_t d,class T>
    void gsDPatchGeom<d,T>::computeSmoothMatrix2()
    {
