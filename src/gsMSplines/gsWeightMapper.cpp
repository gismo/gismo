...
void gsWeightMapper::mapToTargetCoefs(gsMatrix<weightType> const & sourceCoefs,gsMatrix<weightType> & targetCoefs) const
{
    ...
    //    gsSparseSolver<gsWeightMapper::LToGMatrix>::QR solver;
    ...
    if(!m_optimizationMatrix)
        optimize(gsWeightMapper::optTargetToSource);
    // solve system with least squares method
    gsSparseSolver<real_t>::QR solver;
...
void gsWeightMapper::sourceToTarget(indexType source, IndexContainer & target, WeightContainer & weights) const
{
    target.clear();
    weights.clear();
    // todo: add reserve
    for(InIterMat it(m_matrix,source);it;++it)
    {
...
void gsWeightMapper::targetToSource(indexType target,IndexContainer & indizes,WeightContainer & weights) const
{
    indizes.clear();
    weights.clear();
    // todo: add reserve
...
void getNonZeroInnerOfMatrix ( matrixT const &mat, gsWeightMapper::IndexContainer const & outer, gsWeightMapper::IndexContainer & inner)
{
...
void gsWeightMapper::fastSourceToTarget(IndexContainer const & source,IndexContainer & target) const
{
    ...
    getNonZeroInnerOfMatrix(m_matrix,source,target);
}

void gsWeightMapper::fastTargetToSource(const IndexContainer &target, IndexContainer &source) const
{
    ...
    getNonZeroInnerOfMatrix(*m_optimizationMatrix,target,source);
}


void gsWeightMapper::getLocalMap (IndexContainer const & source, IndexContainer const & target, gsMatrix<real_t> &map) const
{
    GISMO_ASSERT(m_matrix.isCompressed(),"optimize() must be called on the mapper with fastSourceToTarget flag before using this function.");
...
void gsWeightMapper::getLocalMap (IndexContainer const & source, gsMatrix<real_t> &map) const
{
    GISMO_ASSERT(m_matrix.isCompressed(),"optimize() must be called on the mapper with fastSourceToTarget flag before using this function.");
