...
class gsDPatchGeom : public gsMappedSpline<d,T>
{
    ...
    public:
        ...
        /// Shared pointer for gsDPatchGeom
        typedef memory::shared_ptr< gsDPatchGeom > Ptr;

        /// Unique pointer for gsDPatchGeom
        typedef memory::unique_ptr< gsDPatchGeom > uPtr;
        ...
        /// Default constructor
        gsDPatchGeom(gsMultiPatch<T> const & mp) ;

        /// Copy constructor
        gsDPatchGeom( const gsDPatchGeom& other );

        using Base::m_compBasis;

        GISMO_CLONE_FUNCTION(gsDPatchGeom)

        virtual ~gsDPatchGeom();
...
#include GISMO_HPP_HEADER(gsDPatchGeom.hpp)
#endif
