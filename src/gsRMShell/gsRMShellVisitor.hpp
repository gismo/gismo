...
namespace gismo
{
        ...
        const gsMatrix<T>& quNodes)
    {
        ...
        // 2.2 Compute norm on Greville points 计算 Greville 点的法向量
        ...
        // 壳四条边上的均布载荷需要计算在参数坐标为0或1时的基函数值(假设两点高斯积分)
        ...
        index_t rr)
    {
            ...
            temp_a2.setZero();
            temp_area.setZero();
            real_t mapArea = 0.0; // 载荷作用面的表面积

            // 作用于模型表面的均布载荷
            if (m_pressure[rr].section == "SUR")
            {   
                // dots对应z方向积分点，0计算下表面，1计算上表面，只能选1个
                if (dots == 1)
                {
                    // cross product 
        ...
        gsVector<T> const& quWeights)
    {
        ...
        if (STRAINS)
        {
            // 应力应变数据外推给 gsRMShellAssembler.hpp
