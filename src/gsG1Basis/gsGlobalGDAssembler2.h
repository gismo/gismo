...
#include <gsG1Basis/gsVisitorGlobalApproxGD2.h>
#include <gsAssembler/gsAssembler.h>

namespace gismo
{

template <class T, class bhVisitor = gsVisitorGlobalApproxGD2<T> >
class gsGlobalGDAssembler2 : public gsAssembler<T>
{
public:
    typedef gsAssembler<T> Base;

public:
    gsGlobalGDAssembler2(gsMultiPatch<T> const & mp,
                         gsBSplineBasis<T> const & basis,
                         gsG1OptionList optionList)
        : m_mp(mp), m_g1OptionList(optionList)
    {
...
void gsGlobalGDAssembler2<T, bhVisitor>::refresh()
{
    // 1. Obtain a map from basis functions to matrix columns and rows
    gsVector<> size(1);
    size.at(0) = m_basis[0].size() * 4; // *4 bcs of alpha^S and beta^S
...
template <class T, class bhVisitor>
void gsGlobalGDAssembler2<T, bhVisitor>::assemble(gsMatrix<> & old_sol)
{
    m_old_sol = old_sol;
...
template <class T, class bhVisitor>
inline void gsGlobalGDAssembler2<T, bhVisitor>::apply(bhVisitor & visitor)
{
#pragma omp parallel
    {
        bhVisitor
...
#endif
        {
            ...
            visitor_.evaluate(m_basis, quNodes, m_mp, m_old_sol, m_g1OptionList);
