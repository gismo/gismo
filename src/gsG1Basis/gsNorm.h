...
//typedef typename NormVisitor::Scalar_t T;
    ...
    */
    ...
    template <class NormVisitor>
    void applyb(NormVisitor & visitor, bool storeElWise = false, boundaryInterface iFace = boundary::none)
    {
        ...
        const gsFunction<T> & func1  = field1->function(L);
        const gsFunction<T> & func2  = field1->function(R);

        // Obtain an integration domain
        const gsBasis<T> & dom = field1->isParametrized() ?
                                 field1->igaFunction(L).basis() : field1->patch(L).basis();

        boxSide side_L = iFace.second();
        boxSide side_R = iFace.first();

        // Initialize visitor
        visitor.initializeb(dom, QuRule_L, evFlags, side_L);
        visitor.initializeb(dom, QuRule_R, evFlags, side_R);

        // Initialize geometry evaluator
        typename gsGeometryEvaluator<T>::uPtr geoEval_L(getEvaluator(evFlags, patchesPtr->patch(L)));
        // Initialize geometry evaluator
        typename gsGeometryEvaluator<T>::uPtr geoEval_R(getEvaluator(evFlags, patchesPtr->patch(R)));

        typename gsBasis<T>::domainIter domIt_L = dom.makeDomainIterator(side_L);
        typename gsBasis<T>::domainIter domIt_R = dom.makeDomainIterator(side_L);
        domIt_R->good();
        for (; domIt_L->good(); domIt_R->next())
        {
            domIt_R->next();
            // Map the Quadrature rule to the element
            ...
            // Evaluate on quadrature points
            visitor.evaluateb(*geoEval_L, func1, quNodes_L, *geoEval_R, func2, quNodes_R);

            // Accumulate value from the current element (squared)
            const T result = visitor.computeb(*domIt_L, *geoEval_L,side_L ,*domIt_R, *geoEval_R, quWeights, m_value);
            if ( storeElWise )
                m_elWise.push_back( visitor.takeRoot(result) );
        }
