...
{
template <class T>
class gsVisitorG1BasisLocal_mp
{
public:

    gsVisitorG1BasisLocal_mp()
    {
    }
                         ...
                         const bool & local)
    {
        ...
        gsMatrix<unsigned> actives_tilde, actives_bar;
        basis_tilde.active_into(md.points.col(0), actives_tilde);
        basis_bar.active_into(md.points.col(0), actives_bar);
        ...
        boundaryInterface m_iFace = gluingData.get_iFace();
        ...
        {
            if (m_iFace.second().side().index() <= 2) // Left, west or east
            {
                gluingData.get_alpha_tilde_L().eval_into(md.points.bottomRows(1),a_t_L); // v
            ...
            else if (m_iFace.second().side().index() >= 3) // Left, north or south
            {
                gluingData.get_alpha_tilde_L().eval_into(md.points.topRows(1),a_t_L); // u
                gluingData.get_alpha_tilde_R().eval_into(md.points.topRows(1),a_t_R);

                gluingData.get_beta_tilde_L().eval_into(md.points.topRows(1),b_t_L);
                gluingData.get_beta_tilde_R().eval_into(md.points.topRows(1),b_t_R);
            }
        }

        if (m_iFace.second().side().index() >= 3) // Left, north or south
        {
            if (m_iFace.second().side().index() == 3) // Left, south
            {
                basis_geo.evalSingle_into(0,md.points.bottomRows(1),N_0_L); // v
                basis_geo.evalSingle_into(1,md.points.bottomRows(1),N_1_L); // v

                basis_geo.evalSingle_into(basis_geo.size()-1,md.points.bottomRows(1),N_0_R); // v
                basis_geo.evalSingle_into(basis_geo.size()-2,md.points.bottomRows(1),N_1_R); // v
            }
            else if (m_iFace.second().side().index() == 4) // Left, north
            {
                basis_geo.evalSingle_into(basis_geo.size()-1,md.points.bottomRows(1),N_0_L); // v
                basis_geo.evalSingle_into(basis_geo.size()-2,md.points.bottomRows(1),N_1_L); // v

                basis_geo.evalSingle_into(0,md.points.bottomRows(1),N_0_R); // v
                basis_geo.evalSingle_into(1,md.points.bottomRows(1),N_1_R); // v
            }
        }
        else if (m_iFace.second().side().index() <= 2) // Left, west or east
        {
            if (m_iFace.second().side().index() == 1) // Left, west
            {
                basis_geo.evalSingle_into(0,md.points.topRows(1),N_0_L); // u
                basis_geo.evalSingle_into(1,md.points.topRows(1),N_1_L); // u

                basis_geo.evalSingle_into(basis_geo.size()-1,md.points.topRows(1),N_0_R); // u
                basis_geo.evalSingle_into(basis_geo.size()-2,md.points.topRows(1),N_1_R); // u
            }
            else if (m_iFace.second().side().index() == 2) // Left, east
            {
        ...
        for (index_t i = 0; i < n_tilde; i++)
        {
            ...
            {
                basis_tilde.evalSingle_into(i,md.points.topRows(1),N_i_t);
                basis_tilde.derivSingle_into(i,md.points.topRows(1),der_N_i_t);
            }
        ...
        for (index_t i = 0; i < n_bar; i++)
        {
            ...
            {
                basis_bar.evalSingle_into(i,md.points.topRows(1),N_j_b);
            }
