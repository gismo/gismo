...
template <class T>
class gsVisitorGlobalGD_mp
{
public:

    gsVisitorGlobalGD_mp()
    {
    }
                         ...
                         index_t & gamma)
    {
        ...
        if (iFace.first().side().index() >= 3) // North and south
        {
            if (iFace.first().side().index() == 4) // North
                uvR.setOnes(2,md.points.cols());
            if (iFace.first().side().index() == 3) // South
                uvR.setZero(2,md.points.cols());
            uvR.topRows(1) = md.points;
        }
        else if (iFace.first().side().index() <= 2) // East and west
        {

            if (iFace.first().side().index() == 2) // East
                uvR.setOnes(2, md.points.cols());
            if (iFace.first().side().index() == 1) // West
                uvR.setZero(2, md.points.cols());
        ...
        for(index_t i = 0; i < uvL.cols(); i++)
        {
            ...
            D0 = ev2.col(1-nulleins);
            real_t D1 = 1/ D0.norm();
            uvL(0,i) = gamma * D1 * D1 * ev2.col(nulleins).transpose() * ev2.col(1-nulleins);
        }
        ...
        {
            PR.jacobian_into(uvR.col(i),ev1);
            D0 = ev1.col(1-nulleins);
            real_t D1 = 1/ D0.norm();
            uvR(0,i) = - gamma * D1 * D1 * ev1.col(nulleins).transpose() * ev1.col(1-nulleins);
        }
