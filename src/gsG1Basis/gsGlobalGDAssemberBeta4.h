...
#include <gsG1Basis/gsVisitorGlobalApproxGDBeta4.h>
...
template <class T, class bhVisitor = gsVisitorGlobalApproxGDBeta4<T> >
class gsGlobalGDAssemblerBeta4 : public gsAssembler<T>
{
public:
    typedef gsAssembler<T> Base;

public:
    gsGlobalGDAssemblerBeta4(gsMultiPatch<T> const & mp,
                         gsBSplineBasis<T> const & basis,
                         std::vector<gsBSpline<T>> const & alpha_S,
                         gsG1OptionList optionList)
        : m_mp(mp), m_alpha_S(alpha_S), m_optionList(optionList)
    {
        m_basis.push_back(basis); // Beta^L
        m_basis.push_back(basis); // Beta^R
...
protected:
    ...
    // alpha^S
    std::vector<gsBSpline<>> m_alpha_S;
...
void gsGlobalGDAssemblerBeta4<T, bhVisitor>::refresh()
{
...
template <class T, class bhVisitor>
void gsGlobalGDAssemblerBeta4<T, bhVisitor>::assemble()
{
...
template <class T, class bhVisitor>
inline void gsGlobalGDAssemblerBeta4<T, bhVisitor>::apply(bhVisitor & visitor)
{
#pragma omp parallel
    {
        bhVisitor
...
#endif
        {
            ...
            visitor_.evaluate(m_basis, quNodes, m_mp, m_alpha_S, m_optionList);
