...
template <class T>
class gsVisitorGlobalGDNorm3
{

public:

    gsVisitorGlobalGDNorm3(index_t p = 2)
    {
        m_p = p;
    }
    ...
    void evaluate(const gsMultiPatch<T> & mp,
                  std::vector<gsBSplineBasis<T>>      & basis,
                  ...
                  gsMatrix<T>            & quNodes)
    {
        ...
        tilde_temp = basis[0].makeGeometry(sol.block(0,0,numBasisFunctions[1],1));
        gsBSpline<T> alpha_L = dynamic_cast<gsBSpline<T> &> (*tilde_temp);

        tilde_temp = basis[1].makeGeometry(sol.block(numBasisFunctions[1],0,numBasisFunctions[2] - numBasisFunctions[1],1));
        gsBSpline<T> alpha_R = dynamic_cast<gsBSpline<T> &> (*tilde_temp);

        tilde_temp = basis[2].makeGeometry(sol.block(numBasisFunctions[2],0,numBasisFunctions[3] - numBasisFunctions[2],1));
        ...
        f1vals.row(0) = a_R.cwiseProduct(DvFL.row(0)) + a_L.cwiseProduct(DuFR.row(0)) + b.cwiseProduct(DvFR.row(0));
        f1vals.row(1) = a_R.cwiseProduct(DvFL.row(1)) + a_L.cwiseProduct(DuFR.row(1)) + b.cwiseProduct(DvFR.row(1));
        // ++++++++++++++++++++++++++++++++
