...
#include <gsG1Basis/gsVisitorGlobalGDNorm3.h>
 ...
 * \ingroup Assembler
*/
template <class T = real_t, class Visitor = gsVisitorGlobalGDNorm3<T> >
class gsGlobalGDNorm3
{

public:

    gsGlobalGDNorm3(const gsMultiPatch<> & multiPatch,
                   gsBSplineBasis<> & basis,
                   gsBSplineBasis<> & basis_beta,
                   const gsMatrix<> & sol )
        :  m_mp(multiPatch), m_sol(sol)
    {
        p = 2;
