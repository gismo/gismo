...
#include <gsG1Basis/gsVisitorGlobalGDNorm4.h>
 ...
 * \ingroup Assembler
*/
template <class T = real_t, class Visitor = gsVisitorGlobalGDNorm4<T> >
class gsGlobalGDNorm4
{

public:

    gsGlobalGDNorm4(const gsMultiPatch<> & multiPatch,
                    const std::vector<gsBSpline<>> & alpha,
                    const std::vector<gsBSpline<>> & beta )
        :  m_mp(multiPatch), m_alpha_S(alpha), m_beta_S(beta)
    {
        p = 2;
    }
...
#pragma omp parallel
        ...
        const int nt  = omp_get_num_threads();
...
#endif
            {
                ...
                visitor.evaluate(m_mp, m_alpha_S, m_beta_S, quNodes);
...
protected:
    ...
    const std::vector<gsBSpline<>> m_alpha_S, m_beta_S;
