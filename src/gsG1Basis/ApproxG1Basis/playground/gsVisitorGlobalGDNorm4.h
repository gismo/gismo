...
template <class T>
class gsVisitorGlobalGDNorm4
{

public:

    gsVisitorGlobalGDNorm4(index_t p = 2)
    {
        m_p = p;
    }
    ...
    void evaluate(const gsMultiPatch<T> & mp,
                  const std::vector<gsBSpline<T>>      & alpha,
                  const std::vector<gsBSpline<T>>      & beta,
                  gsMatrix<T>            & quNodes)
    {


        gsMatrix<> a_L, a_R, b_L, b_R;
        alpha[0].eval_into(quNodes,a_L);
        alpha[1].eval_into(quNodes,a_R);

        beta[0].eval_into(quNodes,b_L);
        beta[1].eval_into(quNodes,b_R);
        // ++++++++++++++++++++++++++++++++
        ...
        f1vals.row(0) = a_R.cwiseProduct(DvFL.row(0)) + a_L.cwiseProduct(DuFR.row(0)) + (b_L.cwiseProduct(a_R) + b_R.cwiseProduct(a_L)).cwiseProduct(DvFR.row(0));
        f1vals.row(1) = a_R.cwiseProduct(DvFL.row(1)) + a_L.cwiseProduct(DuFR.row(1)) + (b_L.cwiseProduct(a_R) + b_R.cwiseProduct(a_L)).cwiseProduct(DvFR.row(1));

        //gsInfo << " NORM : " << a_R.cwiseProduct(DvFL.row(1)) + a_L.cwiseProduct(DuFR.row(1)) << "\n";
        //gsInfo << " NORM 2: " << a_R.cwiseProduct(DvFL.row(0)) + a_L.cwiseProduct(DuFR.row(0))  << "\n";
        // ++++++++++++++++++++++++++++++++
