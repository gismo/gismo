...
#include <gsG1Basis/gsVisitorGlobalApproxGD3Lambda.h>
#include <gsAssembler/gsAssembler.h>

namespace gismo
{

template <class T, class bhVisitor = gsVisitorGlobalApproxGD3Lambda<T> >
class gsGlobalGDAssembler3Lambda : public gsAssembler<T>
{
public:
    typedef gsAssembler<T> Base;

public:
    gsGlobalGDAssembler3Lambda(gsMultiPatch<T> const & mp,
                         gsBSplineBasis<T> const & basis,
                         gsBSplineBasis<T> const & basis_beta,
                         gsG1OptionList optionList)
        : m_mp(mp), m_optionList(optionList)
    {
        m_basis.push_back(basis);
        m_basis.push_back(basis);
        m_basis.push_back(basis_beta);
    ...
    void assemble(gsMatrix<> & old_sol);
...
void gsGlobalGDAssembler3Lambda<T, bhVisitor>::refresh()
{
    // 1. Obtain a map from basis functions to matrix columns and rows
    gsVector<> size;
    size.setZero(1);
    for (size_t i = 0; i < m_basis.size(); i++)
        size.at(0) += m_basis[i].size(); // *4 bcs of alpha^S and beta^S
...
template <class T, class bhVisitor>
void gsGlobalGDAssembler3Lambda<T, bhVisitor>::assemble(gsMatrix<> & old_sol)
{
    m_old_sol = old_sol;
...
template <class T, class bhVisitor>
inline void gsGlobalGDAssembler3Lambda<T, bhVisitor>::apply(bhVisitor & visitor)
{
#pragma omp parallel
    {
        bhVisitor
...
#endif
        {
            ...
            visitor_.evaluate(m_basis, quNodes, m_mp, m_old_sol, m_optionList);
