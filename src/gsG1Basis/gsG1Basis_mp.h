...
#include <gsG1Basis/gsVisitorG1Basis_mp.h>

namespace gismo
{
template<class T, class bhVisitor = gsVisitorG1Basis_mp<T> >
class gsG1Basis_mp : public gsAssembler<T>
{
public:
    typedef gsAssembler<T> Base;

public:
    gsG1Basis_mp(gsMultiPatch<T> const & mp,
                 gsMultiBasis<T> & mb,
        ...
        : m_mp(mp), m_mb(mb), m_iFace(iFace), p_tilde(p_t), r_tilde(r_t), direct(dir), local(loc), plot(pl)
    {
        ...
        gsBSplineBasis<> basis_tilde(kv_tilde);
        for (index_t i = max_degree+1; i < basis_temp_R.knots().size() - (max_degree+1); i = i+(max_degree-m_r))
        {
        ...
        // TODO knotvector should be insertKnot
        if (m_iFace.first().side().index() >= 3) // north or south of BOTH patches
        {
...
void gsG1Basis_mp<T,bhVisitor>::constructSolution(gsMultiPatch<T> & result_L, gsMultiPatch<T> & result_R)
{
...
template <class T, class bhVisitor>
void gsG1Basis_mp<T,bhVisitor>::refresh()
{
...
template <class T, class bhVisitor>
void gsG1Basis_mp<T,bhVisitor>::assemble()
{
...
template <class T, class bhVisitor>
void gsG1Basis_mp<T,bhVisitor>::apply(bhVisitor & visitor, int patchIndex, boxSide side)
{
#pragma omp parallel
    {
...
template <class T, class bhVisitor>
void gsG1Basis_mp<T,bhVisitor>::solve()
{
