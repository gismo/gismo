...
struct ExpressionTraits<InnerProductExpression<_LhsExpr, _RhsExpr, _LhsOrder, _RhsOrder, _LhsSpace, _RhsSpace>>
{
    ...
    static constexpr size_t Space = SpaceType::None;  // Result has no space dependency
...
public:
    ...
    {
        ExpressionValue<typename Base::Scalar> lhs_eval = this->lhs_expr_.eval(k);
        ExpressionValue<typename Base::Scalar> rhs_eval = this->rhs_expr_.eval(k);
        ...
        ExpressionValue<typename Base::Scalar> result_val(1, 1);
    ...
    eval(const index_t k) const
    {
        ...
        ExpressionValue<typename Base::Scalar> result_val(lhs_eval.rowCardinality(), lhs_eval.colCardinality());
    ...
    // --- eval() specialization 4: Matrix ddot (Order=2), Space=None ---
    template <size_t LO = _LhsOrder, size_t RO = _RhsOrder, size_t LS = _LhsSpace, size_t RS = _RhsSpace>
    ...
    eval(const index_t k) const
    {
        ...
        result(0, 0) = (lhs_mat.array()*rhs_mat.array()).sum().matrix();
        return result;
    }

    // --- eval() specialization 5: Matrix ddot (Order=2), Space=Test ---
    template <size_t LO = _LhsOrder, size_t RO = _RhsOrder, size_t LS = _LhsSpace, size_t RS = _RhsSpace>
    ...
    eval(const index_t k) const
    {
        ...
        for (index_t i = 0; i < result.rowCardinality(); ++i)
            for (index_t j = 0; j < result.colCardinality(); ++j)
            {
                ...
                result(i, j) = (lhs_mat.array()*rhs_mat.array()).sum().matrix();
            }
        
        return result;
    }

    // --- eval() specialization 6: Matrix ddot (Order=2), Space=Trial ---
    template <size_t LO = _LhsOrder, size_t RO = _RhsOrder, size_t LS = _LhsSpace, size_t RS = _RhsSpace>
