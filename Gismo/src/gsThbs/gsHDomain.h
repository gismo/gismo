...
/**
   \brief
    Class with a Hierarchical domain structure represented by a box
    kd-tree
...
public: // TO DO private
    ...
    unsigned max_ins_level;

public:

    gsHDomain() : m_index_level(0) 
    {
        m_root=NULL; 
        max_ins_level = 0;
    }
...
*/
    ...
    gsHDomain( const gsHDomain & o) : 
        ...
        m_index_level(o.m_index_level),
        max_ins_level(o.max_ins_level)
    {
    ...
    { 
        ...
        for (unsigned i=0; i<d; ++i) 
            m_upperIndex[i] = (upp[i]<< m_index_level);
...
public:
    ...
    /// The insert function which insert box defined by points [k1, k2] to level lvl.
    /// [k1, k2] are in the incices of lvl
    /// \param k1 the lower left corner of the box
    /// \param k2 the upper right corner of the box
    /// \param *leaf the current node
    /// \param lvl the desired level
    // TO DO: rename laef to node
    void insertBox (point const & k1, point const & k2,
                     node * leaf, int lvl);

    void insertBox (point const & k1, point const & k2, int lvl)
    { insertBox(k1, k2, m_root, lvl); }
    ...
    {
        for ( unsigned i = 0; i!=d; ++i )
            internal_idx[i] = point_idx[i] << (m_index_level-lvl) ;
    }

    /// Returns true if [k1,k2] is completely contained in level and
    /// does not overlap with omegas of higher level
    /// \param k1 the lower left corner of the box
    /// \param k2 the upper right corner of the yebox
    /// \param level the level to be checked against
    /// \param *leaf the current node
    // TO DO "leaf" is misleading -- rename to node_
    bool query1 (point const & k1, point const & k2,
                 int level, node * _node ) const
        { return boxSearch< query1_visitor >(k1,k2,level,_node); }

    bool query1 (point const & k1, point const & k2,
                 int level) const
        { return boxSearch< query1_visitor >(k1,k2,level,m_root); }

    /// Returns true if [k1,k2] is completely contained in omega of
    /// higher level than level
    /// \param k1 lower left corner of the cube [k1,k2]
    /// \param k2 upper right corner of the cube [k1,k2]
    /// \param level current level
    // TO DO "leaf" is misleading -- rename to node_
    bool query2 (point const & k1, point const & k2, 
                 int level, node *_node ) const
    { return boxSearch< query2_visitor >(k1,k2,level,_node); }

    bool query2 (point const & k1, point const & k2, 
                 int level) const
    { return boxSearch< query2_visitor >(k1,k2,level,m_root); }
    ...
    int query3(point const & k1, point const & k2, 
               int level) const
    { return boxSearch< query3_visitor >(k1,k2,level,m_root); }

    /// query4 returns the highest level with which box [k1, k2]
    /// overlaps
    int query4(point const & k1, point const & k2, 
               int level, node  *_node) const
    { return boxSearch< query4_visitor >(k1,k2,level,_node); }

    int query4(point const & k1, point const & k2, 
               int level) const
    { return boxSearch< query4_visitor >(k1,k2,level,m_root); }
    ...
    void incrementLevel()
    {
        ...
        GISMO_ASSERT( max_ins_level <= m_index_level, 
    ...
    {
        return literator(m_root, m_index_level);
    }

    const_literator beginLeafIterator() const
    {
        return const_literator(m_root, m_index_level);
    }
    ...
    {
        return (m_upperIndex[k] >> (m_index_level - lvl) );
    }
    ...
    getPoly(std::vector<gsSegment<T> >& seg) const;
...
*/
    std::vector<std::vector< std::vector<unsigned int > > > getPolylinesSingleLevel(std::vector<gsSegment<T> >& seg) const;
...
private:
    ...
    /// Returns true if box1 is containeed in box2
    ...
    /// Sets the level of all descendents of the node *leaf to level
    /// lvl if the level of the node is smaller then lvl.
    /// Otherwise the level of node is not changed
    /// \param *leaf the node whose descendents are to be set their levels
    ...
    void getRidOfOverlaps( std::list< std::list< gsSegment<T> > >& vert_seg_lists ) const;

    /// Sweepline algorithm.
    void sweeplineConnectAndMerge( std::vector< std::vector< std::vector<unsigned int> > >& result,
                                   std::list< std::list< gsSegment<T> > >& vert_seg_lists ) const;
