...
gsHDomain<d,T>::insertBox ( point const & k1, point const & k2,
                             node *_node, int lvl)
{
    GISMO_ENSURE( lvl <= static_cast<int>(m_index_level), "Max index level reached..");
...
*/
        if ( curNode->isLeaf() ) // reached a leaf
        {
            ...
            node * newLeaf = curNode->adaptiveAlignedSplit(iBox, m_index_level);
    ...
    if ( static_cast<unsigned>(lvl) > max_ins_level) 
        max_ins_level = lvl;
}
...
*/
    ...
    std::vector<std::vector<gsSegment<T> > > seg;
    seg.resize(max_ins_level+1);

    // For each level prepare the vertical lines separately
    for (unsigned int i = 0; i < boxes.size() ; i ++)
    {
        seg[boxes[i][4]].push_back(gsSegment<unsigned int>(boxes[i][0],boxes[i][1],boxes[i][3], false) );
        seg[boxes[i][4]].push_back(gsSegment<unsigned int>(boxes[i][2],boxes[i][1],boxes[i][3], false) );
    }

    // Process vertical lines from each level separately
    std::vector< std::vector<std::vector< std::vector<unsigned int > > > > result;
    for(unsigned int i = 0; i < max_ins_level+1; i++)
    {
...
template<unsigned d, class T>
std::vector<std::vector< std::vector<unsigned int > > > gsHDomain<d,T>::getPolylinesSingleLevel(std::vector<gsSegment<T> >& seg) const
{
    ...
    std::list< std::list< gsSegment< T > > > vert_seg_lists;
    ...
    std::list< gsSegment< unsigned int > > segs_x; // segments with the same particular x coord
    for( typename std::vector< gsSegment< T > >::const_iterator it_seg = seg.begin(); it_seg != seg.end(); ++it_seg )
    {
        if( segs_x.empty() || (*it_seg).get_m_x() == segs_x.front().get_m_x() )
...
template <unsigned d, class T>
void gsHDomain<d,T>::getRidOfOverlaps( std::list< std::list< gsSegment<T> > >& vert_seg_lists ) const
{
    bool need_to_erase = false;
    for( typename std::list< std::list< gsSegment<T> > >::iterator it_x = vert_seg_lists.begin(); it_x != vert_seg_lists.end(); )
    {
        // For each x coordinate substract overlapping segments.
        for( typename std::list< gsSegment<T> >::iterator it_slow = (*it_x).begin(); it_slow != (*it_x).end(); ++it_slow)
        {
            // Don't we need to start the next already at (*it_x).begin() ?
            for( typename std::list< gsSegment<T> >::iterator it_quick = it_slow; it_quick != (*it_x).end(); ++it_quick)
            {
                if( it_quick != it_slow )
                {
        ...
        //if( need_to_erase )
        //{
        for( typename std::list< gsSegment<T> >::iterator it_slow = (*it_x).begin(); it_slow != (*it_x).end();)
        {
            if( (*it_slow).length() == 0 )
            {
                (*it_x).erase( it_slow++ );
...
template <unsigned d, class T>
void gsHDomain<d,T>::sweeplineConnectAndMerge( std::vector< std::vector< std::vector<unsigned int> > >& result, std::list< std::list< gsSegment<T> > >& vert_seg_lists ) const
{
    /*========================
    ...
    std::list< gsPolyline<T> > act_poly;
    std::queue< gsPolyline<T> > poly_queue;

    for( typename std::list< std::list< gsSegment<T> > >::iterator it_x = vert_seg_lists.begin(); it_x != vert_seg_lists.end(); ++it_x )
    {
        ...
        for( typename std::list< gsPolyline<T> >::const_iterator it = act_poly.begin(); it != act_poly.end(); ++it )
            poly_queue.push( *it );

        act_poly.erase( act_poly.begin(), act_poly.end() );

        gsPolyline<unsigned int> curr_poly;
        while( !poly_queue.empty() )
        {
            curr_poly = poly_queue.front();
            poly_queue.pop();
            bool is_active = true;
            for( typename std::list< gsSegment<T> >::iterator it_seg = (*it_x).begin(); it_seg != (*it_x).end(); )
            {
        ...
        for( typename std::list<gsSegment<T> >::const_iterator it = (*it_x).begin(); it != (*it_x).end(); ++it )
        {
            gsPolyline<T> new_polyline( *it );
        ...
        for( typename std::list< gsPolyline<T> >::iterator it_slow = act_poly.begin(); it_slow != act_poly.end(); ++it_slow )
        {
            for( typename std::list< gsPolyline<T> >::iterator it_quick = it_slow; it_quick != act_poly.end(); ++it_quick)
            {
        ...
        for( typename std::list< gsPolyline<T> >::iterator it = act_poly.begin(); it != act_poly.end(); )
        {
            if( (*it).almostClosed() )
            {
...
{
    for ( unsigned i = 0; i!=d; ++i )
        result[i] = index[i] << (max_ins_level-lvl) ;
}
...
{
    for ( unsigned i = 0; i!=d; ++i )
        result[i] = index[i] << (m_index_level-lvl) ;
}
...
{
    for ( unsigned i = 0; i!=d; ++i )
        result[i] = index[i] >> (this->m_index_level-lvl) ;
}
}// end namespace gismo
