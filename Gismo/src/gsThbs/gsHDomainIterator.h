...
/** Re-implements gsDomainIterator for iteration over all elements of a <b>hierarchical</b> parameter domain.\n
...
private:
    ...
    bool nextBox()
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                if (cnode->right != NULL)
                {
                    std::cout << "This should not happen in this version of "
                                 "KD-tree: cnode is not a leaf." << std::endl;
        ...
        std::cout << "Something horribly wrong happend." << std::endl;
        return false;
    }
    ...
    void setNotAlignedMesh()
    {
        ...
        for (unsigned dim = 0; dim < d; dim++)
        {
            ...
            const gsCompactKnotVector<T> & kv =
                    m_bases[m_tree.m_index_level]->component(dim).knots();
    ...
    {
        const int level = m_cnode->level;
        const int maxLevel = static_cast<int>(m_tree.m_index_level);
