...
// version according to NURBS Book, Algorithm A2.1 (p. 68)
template <class T>
...
{
  typename std::vector<T>::const_iterator itr;
  result.resize( 1,this->size() - my->p - 1 ) ; 
  unsigned i(0);
  
  if ( my->p!=0)
    for ( itr= my->knots.begin(); itr != my->knots.end()-my->p-1; ++itr )
      result(0,i++)=  std::accumulate( itr+1, itr+my->p+1, T(0) ) / my->p ;
  else
    for ( itr= my->knots.begin(); itr != my->knots.end()-1; ++itr )
