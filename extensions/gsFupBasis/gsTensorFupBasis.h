/** @file gsTensorFupBasis.h

    @brief Provides declaration of TensorFupBasis abstract interface.
...
/** 
    @brief A tensor product Fup basis.
...
class gsTensorFupBasis : public gsTensorBasis<d,T>
{
public: 
    ...
    /// Family type
    typedef gsFupBasis<T>  Family_t;

    typedef gsTensorFupBasis Self_t;

    /// Coordinate basis type
    typedef gsFupBasis<T> CoordinateBasis;
    ...
    typedef typename gsFupTraits<d,T>::Geometry GeometryType;

    /// Associated Boundary basis type
    //typedef typename gsFupTraits<static_cast<short_t>(d-1),T>::Basis BoundaryBasisType;

    typedef typename Base::iterator        iterator;
    typedef typename Base::const_iterator  const_iterator;

    /// Smart pointer for gsTensorFupBasis
...
public:
    ...
    gsTensorFupBasis() : Base()
    {
        for(short_t i = 0; i!=d; ++i)
            this->m_bases[i] = new Basis_t();
    }

    void swap(gsTensorFupBasis & other)
    {
        this->Base::swap(static_cast<Base&>(other));
    }
    
#if !EIGEN_HAS_RVALUE_REFERENCES
    gsTensorFupBasis & operator=(gsTensorFupBasis other)
    { gsTensorFupBasis::swap(other); return *this;}
#endif
     ...
     */
    template<typename U>
    gsTensorFupBasis( KnotVectorType KV1, gsKnotVector<U> KV2,
     ...
     */
    gsTensorFupBasis( KnotVectorType KV1, 
                          KnotVectorType KV2, 
                          KnotVectorType KV3 )
    ...
    explicit gsTensorFupBasis(std::vector<KnotVectorType> KV)
    { 
    ...
    gsTensorFupBasis( Basis_t* x,  Basis_t*  y) : Base(x,y) 
    { 
        GISMO_ENSURE(d==2,"Invalid constructor." );
    }
    
    gsTensorFupBasis( Basis_t* x,  Basis_t* y, Basis_t* z ) : Base(x,y,z) 
    { 
        GISMO_ENSURE(d==3,"Invalid constructor." );
    }
    
    gsTensorFupBasis( Basis_t* x,  Basis_t* y, Basis_t* z, Basis_t* w ) : Base(x,y,z,w) 
    { 
        GISMO_ENSURE(d==4,"Invalid constructor." );
    }
    
    gsTensorFupBasis(std::vector< gsBasis<T>*> & bb ) : Base(bb.data())
    {
        GISMO_ASSERT( checkVectorPtrCast<Basis_t>(bb), "Invalid vector of basis pointers.");
        GISMO_ENSURE( d == bb.size(), "Wrong d in the constructor of gsTensorFupBasis." );
        bb.clear();
    }
    
    explicit gsTensorFupBasis(std::vector< Basis_t*> & bb ) 
...
//     typename gsFupTraits<static_cast<short_t>(d-1),T>::Basis::uPtr boundaryBasis(boxSide const & s);
// #endif
...
//     }
    ...
    GISMO_CLONE_FUNCTION(gsTensorFupBasis)
...
public:
    ...
    {
        os << "TensorFupBasis: dim=" << this->dim()<< ", size="<< this->size() <<".";
        //for ( short_t i = 0; i!=d; ++i )
    ...
    ///
    /// \copydetails gsFupBasis::refine_k
    ...
    /// Sets the coefficients so that the resulting TensorFup is periodic in direction dir.
