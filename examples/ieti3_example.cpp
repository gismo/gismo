/** @file ieti3_example.cpp

    @brief Provides an example for the inexact ieti solver
...
int main(int argc, char *argv[])
{
    ...
    // Right-hand-side
    gsFunctionExpr<> f( "2*pi*sin(pi*x)*sin(pi*y)", mp.geoDim() );
    ...
    for (index_t k=0; k<nPatches; ++k)
    {
        ...
        //gsSparseMatrix<> Atilde = gsPatchPreconditionersCreator<>::stiffnessMatrix(mb_local.basis(0), bc_local, opt);
        gsPoissonAssembler<> hatassembler(
        ...
        tmp = ietiMapper.dofMapperLocal(k);
        hatassembler.system() = gsSparseSystem<>(tmp); // gsSparseSystem takes per reference and steals contets
        hatassembler.setFixedDofVector(ietiMapper.fixedPart(k));
        hatassembler.assemble();
        ...
        prec.addSubdomain(
            ...
            gsScaledDirichletPrec<>::schurComplement( blocks, gsPatchPreconditionersCreator<>::fastDiagonalizationOp(mb_local.basis(0), bc_A11, opt) )
        );
        ...
        //gsLinearOperator<>::Ptr basisPrec = gsPatchPreconditionersCreator<>::fastDiagonalizationOp(mb_local.basis(0), bc_local, opt, 0, 1, reg);
        //gsLinearOperator<>::Ptr localMatOp = makeMatrixOp(memory::make_shared<gsSparseMatrix<> >(new gsSparseMatrix<>(modifiedLocalMatrix)));
        //gsLinearOperator<>::Ptr localSolver = gsIterativeSolverOp<gsConjugateGradient<> >::make(localMatOp, basisPrec);

        // Set up the local preconditioners
        for(index_t i = 1; i <= 1<<mb.dim(); i++)
            bc_local.addCornerValue(i, 0, 0);

        gsLinearOperator<>::Ptr localPrec = gsPatchPreconditionersCreator<>::fastDiagonalizationOp(mb_local.basis(0), bc_local, opt, 0, 1, reg);
        ...
        //gsInfo << "basis: " << basisDel.toDense().transpose()<<"\n";

/*
        /// keep identity in the primals
        primal.addContribution(
            jumpMatrix, localMatrix, localRhs,
            gsPrimalSystem<>::primalBasis(
                localSolver,
                embeddingForBasis, rhsForBasis, ietiMapper.primalDofIndices(k), primal.nPrimalDofs()
            ),
            makeMatrixOp(localEmbedding.moveToPtr())
        );

        gsInfo << "basis: " << gsPrimalSystem<>::primalBasis(
                localSolver,
                embeddingForBasis, rhsForBasis, ietiMapper.primalDofIndices(k), primal.nPrimalDofs()
        ).toDense().transpose()<<"\n";
*/
