...
void completeWithIdentity(gsSparseMatrix<> & mat)
{
    ...
    for (index_t k = 0; k!=mat.outerSize(); ++k)
        for(auto it = mat.begin(k); it; ++it)
            nzr.insert(it.row());
    nzr.insert(nzr.end(), mat.outerSize());
...
int main(int argc, char *argv[])
{
    ...
    gsFunctionExpr<> ff("t:=1;cos(t*pi*x) * sin(t*pi*y)", 2);
//    gsFunctionExpr<> ff("1", 2);
    ...
    for( index_t r = 0; r<=numRefine; ++r)
    {
        ...
        fd.read(fn + util::to_string(init_knots++) + ".xml");
        fd.getFirst(mp);
        mp.computeTopology();
        fd.getFirst(mb);
        fd.getFirst(cf);
...
//      u.setup(bc, dirichlet::l2Projection, 0);
        ...
        // Poisson
//        A.assemble( igrad(u,G) * igrad(u,G).tr() * meas(G), - u * lf * meas(G) );
