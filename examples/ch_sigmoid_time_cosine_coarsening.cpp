/** @file cahn-hilliard.cpp
    ...
    //  =========== QUADRATIC BASIS ===========
    ./bin/ch_sigmoid_time_cosine_coarsening -r 7 -t 1e-3 -N 5 -c 0 -v 2 -s 0 -e 1 --plot
...
#   else
    ...
    else
    {
        ...
        real_t error = gsL2Projection<real_t>::project(dbasis,mp,initial_cond,tmp);  // 3rd arg has to be multipatch
    ...
    for (index_t step = 0; step!=maxSteps; step++)
    {
        ...
        // gsInfo << "cvec not notmalized min = " << cvec.minCoeff() << "\n";
        // gsInfo << "cvec not notmalized max = " << cvec.maxCoeff() << "\n\n";
        ...
        // gsInfo << "cInt: min = " << cvec.minCoeff() << ", max = " << cvec.maxCoeff() << "\n";
        // gsInfo<< "Number of elements marked for coarsening: "<<coarsen.totalSize()<<"\n";
