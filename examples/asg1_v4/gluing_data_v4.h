/** @file gluing_data_v4.h

    @brief Header-only AS-G1 gluing data computation (v4).

    Wraps the streamlined v3/v4 gluing data routines into the namespace
    `gismo::asg1v4` so that they may coexist with the example
    translation units (which define the same function names in the
    global namespace).

    Public API (in namespace `gismo::asg1v4`):

      * `det2`, `partial`, `breaksOf`           -- low-level helpers
      * `InterfaceSamples<T>`                   -- POD with D1, D2, D3, t, w
      * `sampleInterface(mp, ifc, flipped, n)`  -- sample determinants
      * `SolveResult<T>`                        -- POD with alpha[4], beta[4], errors
      * `solveLinearGluing(samples)`            -- single LS solve
      * `computeGluingDataForInterface(...)`    -- full driver returning 8 numbers

    Convention (same as gluing_data_v4.cpp):
        result(8) = [a1_0, a1_1, b1_0, b1_1, a2_0, a2_1, b2_0, b2_1]
    in patch-1's tangential parametrisation.
...
template <class T>
inline std::vector<T> breaksOf(const gsGeometry<T>& g, short_t dir)
{
...
/// Return 8 numbers in patch-1 tangential parametrisation:
///     [a1_0, a1_1, b1_0, b1_1, a2_0, a2_1, b2_0, b2_1]
/// If `verbose`, prints a one-line summary.
template <class T>
    ...
    bool verbose = true)
{
    ...
    {
        if (verbose) gsInfo << "  WARNING: D1 or D2 changes sign on the interface.\n";
        return result;
    }
    ...
    if (r.alphaErr >= eps)
    {
        if (verbose) gsInfo << "  FAILED alphaErr=" << r.alphaErr << "\n";
        return result;
    }
    ...
    // Embedding-convention sign flip on beta: the embedding
    // `createGluingDataArgyrisBasis` uses the convention
    //     beta_1*D2 - beta_2*D1 = -D3,
    // while `solveLinearGluing` fits beta_1*D2 - beta_2*D1 = +D3.
    // Negate all beta values to match the embedding convention.
    r.b[0] = -r.b[0]; r.b[1] = -r.b[1];
    r.b[2] = -r.b[2]; r.b[3] = -r.b[3];
...
/// Convenience: assemble (nPatches x 16) matrix.
/// Layout per side s in {west=0, east=1, south=2, north=3}:
///   4*s+0 = alpha_0   4*s+1 = alpha_1
///   4*s+2 = beta_0    4*s+3 = beta_1
/// Boundary sides keep alpha=1, beta=0.
template <class T>
    ...
    bool verbose = false)
{
    ...
    for (auto it = mp.iBegin(); it != mp.iEnd(); ++it)
    {
        ...
        bool ok = false;
        gsVector<T> gd = computeGluingDataForInterface<T>(
            mp, interf, ok, eps, numGaussPerSpan, verbose);
        if (!ok) continue;
