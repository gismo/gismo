...
int main(int argc, char* argv[])
{
    ...
    std::string input("curves3d/quarter_circle.xml");
    std::string output("");
    ...
    // p-refine
    if (numElevate!=0)
        pGeom->degreeElevate(numElevate);
    ...
    gsVector<real_t,3> der1vec = der1.col(0);
    gsVector<real_t,3> der2vec = der2.col(0);

    gsVector<real_t,3> b = der1vec.cross(der2vec).normalized();
    gsInfo<<"\n\nbinormal at u " << ": \n"
              << b<<"\n\n";

    gsVector<real_t,3> n = b.cross(der1vec);//.normalized();
    gsInfo<<"\n\nnormal at u " <<": \n"
            << n<<"\n\n";
    ...
    a2 << 4,5,6;
    a3 << 7,8,9;
    gsDebugVar((a1*a2.transpose())*a3);
    gsDebugVar((a1.transpose()*a2));
