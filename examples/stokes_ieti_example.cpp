...
int main(int argc, char *argv[])
{
    ...
    if (plot)
    {        
        gsInfo << "Write Paraview data to files ieti_velocity_x.pvd, ieti_velocity_y.pvd and ieti_pressure.pvd.\n";
        
        const char* filenames[] = {"ieti_velocity_x", "ieti_velocity_y", "ieti_pressure"};
	GISMO_ENSURE(ietiMapper.size()==(std::size_t)3, "Dimension mismatch.");
        for (std::size_t r=0; r<ietiMapper.size(); ++r)
        {
            gsConstantFunction<> zero(0.0, 2);
            gsPoissonAssembler<> assembler(mp, mb[r], bc[r], zero, dirichlet::elimination, iFace::glue);
            ...
            gsWriteParaview<>( gsField<>( assembler.patches(), mpsol ), filenames[r], 1000);
        }
    }
