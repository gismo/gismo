                  ...
                  index_t & projection_Crs)
{
    if (projection_Crs == 0)
    {
        gsL2Projection<real_t>::project(ibasis, dbasis_coarse,fine,mp,coefs);
    }
...
#   else
    ...
    for (index_t step = 0; step!=maxSteps; step++)
    {
        ...
        for (index_t refIt = 0; refIt!=MESHopt.askInt("RefIt",5); refIt++)
        {
            ...
            // error_ref_cnew = gsL2Projection<real_t>::project(ibasis, dbasis, mp_cnew.patch(0),mp,CnewF);
            // error_ref_dcnew = gsL2Projection<real_t>::project(ibasis, dbasis,mp_dcnew.patch(0),mp,dCnewF);    
