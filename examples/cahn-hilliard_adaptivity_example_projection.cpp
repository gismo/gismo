...
template <class T>
T interpolate_crs(const gsMultiBasis<T> & ibasis,
                  ...
                  index_t & projection_Crs)
{
    if (projection_Crs == 0)
    {
        gsL2Projection<real_t>::projectFunction(ibasis, dbasis_coarse,fine,mp,coefs);
    }
...
#   else
    ...
    for (index_t step = 0; step!=maxSteps; step++)
    {
        ...
        error_crs_c  = interpolate_crs(ibasis, dbasis, mp_cold.patch(0) , CnewF, mp, projection_Crs);
        error_crs_dc = interpolate_crs(ibasis, dbasis, mp_dcold.patch(0), dCnewF, mp, projection_Crs);
        ...
        for (index_t refIt = 0; refIt!=MESHopt.askInt("RefIt",5); refIt++)
        {
            ...
            // In case the mesh is refined, we need to project the previous solution onto the new refined mesh
            // This projection should be exact, since the spaces are nested

            // error_ref_cnew = gsL2Projection<real_t>::projectFunction(ibasis, dbasis, mp_cnew.patch(0),mp,CnewF);
            // error_ref_dcnew = gsL2Projection<real_t>::projectFunction(ibasis, dbasis,mp_dcnew.patch(0),mp,dCnewF);    
            gsQuasiInterpolate<real_t>::localIntpl(dbasis.basis(0),mp_cnew.patch(0),CnewF);
            gsQuasiInterpolate<real_t>::localIntpl(dbasis.basis(0),mp_dcnew.patch(0),dCnewF);
        ...
        if (MESHopt.askSwitch("Adaptive",true))
        {
            ...
            dCnew.resize(w.mapper().freeSize(),1); // freeSize if updated because of the setup
