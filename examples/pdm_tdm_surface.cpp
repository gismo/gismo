...
int main(int argc, char *argv[])
{
    ...
    index_t deg = 2; // d
    // e
    std::string fn = "../filedata/fitting/simple_grid.xml"; // f
    // g, h, i, j, k, l,
    real_t sigma = 1; // m
   ...
   gsWrite(*initGeom, "initGeom");

   gsExprEvaluator<> ev;
   auto G = ev.getMap(*initGeom);
   gsMultiBasis<> mb(initGeom->basis());
   ev.setIntegrationElements(mb);
   ev.writeParaview( sn(G).normalized() , G, "Gnormals");
   ...
   gsWriteParaview(*tdm_obj.result(), "tdm_it0");
   gsWrite(*tdm_obj.result(), "result_it0");
