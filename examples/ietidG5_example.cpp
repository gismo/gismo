...
int main(int argc, char *argv[])
{
    ...
    gsInfo << "Run ietidG5_example with options:\n" << cmd << std::endl;
...
*/
    ...
    if (ietiMapper.nPrimalDofs()>0)
    {
        ...
        gsLinearOperator<>::Ptr solver = makeSparseCholeskySolver(primal.localMatrix());
    ...
    gsConjugateGradient<>( ieti.schurComplement(), prec.preconditioner() )
            ...
            .solveDetailed( ieti.rhsForSchurComplement(), x, errorHistory );
    //! [Solve]
    ...
    gsMatrix<> uVec = ietiMapper.constructGlobalSolutionFromLocalSolutions(
        primal.distributePrimalSolution(
            ieti.constructSolutionFromLagrangeMultipliers(x)
        )
    );
    //! [Recover]
    gsInfo << "done.\n\n";
