...
int main(int argc, char *argv[])
{
    ...
    {
        A.assembleBdr(bc.get("Weak Clamped"), - lambda * igrad(w,G) *  nv(G)  * ilapl(w,G).tr() + // consistency term
    ...
    collection.options().setInt("elementResolution", 4);
        ...
        for (index_t dt_it = 0; dt_it != lmax; dt_it++)
        {
            ...
            for (index_t k = 0; k!=2; k++)
            {
                ...
                for (index_t it = 0; it!= maxIt; it++)
                {
                    ...
                    if (bc.get("Weak Clamped").size()!=0)
                    {
                        A.assembleBdr(bc.get("Weak Clamped"), - igrad(w,G) * nv(G) * lambda * ilapl(c,G).val() // consistency term
                                                         +(igrad(w,G) * nv(G).normalized()) * hmax * penalty * (igrad(c,G) * nv(G))  // penalty term
                                                         - lambda * ilapl(w,G) * igrad(c,G) * nv(G)); // symmetry term
                    }
