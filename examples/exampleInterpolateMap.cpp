/** @file exampleInterpolateMap.cpp
    ...
    - a circle
    ./bin/exampleInterpolateMap -d 1 -X "cos(2*pi*x)" -Y "sin(2*pi*x)" -Z "0"

    - a helical curve
    ./bin/exampleInterpolateMap -d 1 -X "cos(2*pi*x)" -Y "sin(2*pi*x)" -Z "x"

    - a helical surface
    ./bin/exampleInterpolateMap -d 2 -X "x*cos(2*pi*y)" -Y "x*sin(2*pi*y)" -Z "2*pi*y"

    - Hyperboloid
    ./bin/exampleInterpolateMap -d 2 -X "sqrt(1+4*(y-1/2)^2)*cos(2*pi*x)" -Y "4*(y-1/2)" -Z "sqrt(1+4*(y-1/2)^2)*sin(2*pi*x)"  -k 8

    - A bottle
    ./bin/exampleInterpolateMap -d 2 -X "(2+sin(4*pi*y))*cos(2*pi*x)" -Y "10*y" -Z "(2+sin(4*pi*y))*sin(2*pi*x)"

    - Mobius band
    ./bin/exampleInterpolateMap -d 2 -X "(4+2*(y-0.5)*cos(pi*x))*cos(2*pi*x)" -Y "(4+2*(y-0.5)*cos(pi*x))*sin(2*pi*x)" -Z "2*(y-0.5)*sin(pi*x)"

    - An part of an annulus
    ./bin/exampleInterpolateMap -d 2 -X "cos(x)*(y+1)" -Y "sin(x)*(y+1)" -Z "0"

    - A quarter of an annulus
    ./bin/exampleInterpolateMap -d 2 -X "cos(0.5*pi*x)*(y+1.5)" -Y "sin(0.5*pi*x)*(y+1.5)"

    - a cylinder
    ./bin/exampleInterpolateMap -d 2 -X "5*cos(2*pi*y)" -Y "5*sin(2*pi*y)" -Z "15*x"

    - Sphere
    ./bin/exampleInterpolateMap -d 2 -X "cos(pi*x)*cos(2*pi*y)" -Y "sin(pi*x)*cos(2*pi*y)" -Z "sin(2*pi*y)" -k 10

    - Torus
    ./bin/exampleInterpolateMap -d 2 -X "(3+cos(2*pi*y))*cos(2*pi*x)" -Y "(3+cos(2*pi*y))*sin(2*pi*x)" -Z "sin(2*pi*y)" -k 8

    - Cone
    ./bin/exampleInterpolateMap -d 2 -X "cosh(2*pi*x)*cos(2*pi*y)" -Y "cosh(2*pi*x)*sin(2*pi*y)" -Z "sinh(2*pi*x)"

    - 3D pipe
    ./bin/exampleInterpolateMap -d 3 -X "cos(2*pi*x)*(y+2)" -Y "sin(2*pi*x)*(y+2)" -Z "4*z"

    - a volume
    ./bin/exampleInterpolateMap -d 3 -X "cos(x)*(y+1)" -Y "sin(x)*(y+1)" -Z "z*2"

    - a cylindrical volume
    ./bin/exampleInterpolateMap -d 3 -X "x*cos(2*pi*y)" -Y "x*sin(2*pi*y)" -Z "3*z"

    - a degenerate volume
    ./bin/exampleInterpolateMap -d 3 -X "5*cos(2*pi*x)" -Y "5*sin(2*pi*y)" -Z "15*z"

    ./bin/exampleInterpolateMap -d 3 -X "(z+1)*sqrt(1+4*(y-1/2)^2)*cos(2*pi*x)" -Y "(z+1)*4*(y-1/2)" -Z "(z+1)*sqrt(1+4*(y-1/2)^2)*sin(2*pi*x)"  -k 4
    ...
    Author(s): Programmed during the workshop
               http://www.gs.jku.at/gismo/wiki/WikiStart/gs2015
*/
