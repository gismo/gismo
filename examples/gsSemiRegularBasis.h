...
{
    gsKnotVector<T> kv; //(0.0,1.0, 0, deg+1);
    const int numV = std::pow(2, numLayers) + deg;
    m_v.reserve(numV);
    unsigned numKnots;
    ...
    {
        numKnots = (1<<(unsigned)std::floor( std::log2(i+1) ))-1;
        kv.initUniform(0.0, 1.0, numKnots, deg+1, 1, deg);
