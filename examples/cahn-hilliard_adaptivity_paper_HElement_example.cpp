                ...
                const gsOptionList     & options)
{
    ...
    // auto fun1 = ev.getVariable(sourceFunction1, G); // solution before projection      
...
#   else
    ...
    {    
        ...
        Cold.setZero(mp_cold.patch(0).basis().size(),1);
        // mp_dcold.addPatch(dbasis.basis(0).makeGeometry(Cold));
        mp_dcold.addPatch(mp_cold.patch(0).basis().makeGeometry(Cold));
    ...
    for (index_t step = 0; step!=maxSteps; step++)
    {
        ...
        for (index_t refIt = 0; refIt!=maxRefIt; refIt++)
        {
            ...
            real_t bulk_energy       = computeInt(ibasis, mp, bulk_term, A.options()); //ev.integral(meas(G) * bulk_term);
