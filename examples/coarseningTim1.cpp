...
gsGeometry < > ::uPtr TimLocalFitting(gsTHBSplineBasis < 2 > & THB, gsGeometry < > & geom, real_t & error, gsMatrix < real_t > & matrixOfLC, gsMatrix < real_t > & matrixOfUC, gsMatrix < int > & maxLevels, gsMatrix < int > & minLevels,
                                      real_t & au, real_t & bu, real_t & av, real_t & bv, real_t & boundaryerror, gsMatrix < int > basisInd, gsMatrix < > THBcoefs){
    gsMatrix < real_t > unknowns(THB.size(), 2); //it will contain the solution
    gsMatrix < > vectorGaussPoints = computeGaussPoints(THB);
    int numberOfElements = THB.numElements(); //Find number of elements
    gsGaussRule < > gr(THB, 2, 2);
    gsMatrix < > valueGaussPoints(2, gr.numNodes() * basisInd.cols());//Vector for the values of my know function in Gauss points
    gsMatrix < real_t > valueGaussPointsFitting(2, gr.numNodes() * basisInd.cols()); //Vector for the value of the fitted function in Gauss points
    gsMatrix < real_t > punto(2, 1);
    gsMatrix < real_t > qiymet(2, 1);
    gsMatrix < real_t > valore1(1, 1);
    gsMatrix < real_t > valore2(1, 1);
    ...
    gsFitting < > fittingObject(vectorGaussPoints, valueGaussPoints, THB);
...
//    std::string xmlFile = "/home/turing/theydarov/xmlFiles/resultLocalsuccessstuff";
    ...
    real_t epsilon_g = 1e+6, epsilon_f = 1e-2;
...
//            currCellIndex = pickCell(nonCheckedCells, currArrayIndex, levNow, x1U, y1U, x2U, y2U);
            currCellIndex = pickCell(vectorS, currArrayIndex, levNow, x1U, y1U, x2U, y2U);
...
//                        gsInfo << m << "IS PROBLEMATIC\n";
                    ...
                    fittedFunction = myLocalApproximate(THB,
                                                        *geom,
                                                        ...
                                                        basisInd,
                                                        THBcoefs);
                    fittedFunction = TimLocalFitting(THB,
                                                     *geom,
                                                     maxError,
                                                     matrixOfTPLC,
                                                     matrixOfTPUC,
                                                     ...
                                                     basisInd,
                                                     THBcoefs);
                    gsWriteParaview(*fittedFunction, "TimLocal");
                    return 0;
