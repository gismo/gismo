/** @file solid-shell_example.cpp

    @brief Solver for solid-shells
...
int main(int argc, char *argv[])
{
    ...
    gsBoundaryConditions<> bc_d, bc_n;
    bc_d.setGeoMap(mp);
    bc_n.setGeoMap(mp);
    gsVector<> tmp(3);
    tmp << 0, 0, 0;
    ...
    {
        bc_d.addCondition(boundary::north, condition_type::dirichlet, 0, 0, false, i );
        bc_d.addCondition(boundary::east, condition_type::dirichlet, 0, 0, false, i );
        bc_d.addCondition(boundary::south, condition_type::dirichlet, 0, 0, false, i );
        bc_d.addCondition(boundary::west, condition_type::dirichlet, 0, 0, false, i );

        bc_n.addCondition(boundary::north, condition_type::dirichlet, 0, 0, false, i );
        bc_n.addCondition(boundary::east, condition_type::dirichlet, 0, 0, false, i );
        bc_n.addCondition(boundary::south, condition_type::dirichlet, 0, 0, false, i );
        bc_n.addCondition(boundary::west, condition_type::dirichlet, 0, 0, false, i );
    }
    tmp << 0,0,-1;
    //! [Boundary condition case 1]
    ...
    ////////////////// project normals to control points
    gsMultiPatch<> mpN, mpN_def;
    for (size_t p = 0; p!=mp.nPatches(); ++p)
    {
        gsMatrix<> anchors, normals;
        dbasis.basis(p).anchors_into(anchors);
        gsSparseMatrix<> C;
        dbasis.basis(p).collocationMatrix(anchors,C);

        normals.resize(mp.targetDim(),anchors.cols());
        for (index_t k = 0; k!=anchors.cols(); k++)
            normals.col(k) = ev.eval(sn(G),anchors.col(k),p);
        ...
        gsMatrix<> coefs = solver.solve(normals.transpose());

        mpN.addPatch(dbasis.basis(p).makeGeometry(give(coefs)));
    }

    mpN_def = mpN;

    geometryMap N = A.getMap(mpN); // the last map counts
    geometryMap defN = A.getMap(mpN_def);

    ev.writeParaview(N,G,"normals");

    // Set the discretization space
    space u_d = A.getSpace(dbasis, 3);
    space u_n = A.getSpace(dbasis, 3);
    u_d.setup(bc_d, dirichlet::interpolation, 0);
    u_n.setup(bc_n, dirichlet::interpolation, 0);

    // Solution vector and solution variable
    gsMatrix<> random;
    solution u_sol = A.getSolution(u_d,random);
    ...
    **/
    ...
    // Strain
    auto e          = 0.5 * ( flat(jac(defG).tr()*jac(defG)) - flat(jac(G).tr()* jac(G)) ) ;
    auto e_der      = flat( jac(defG).tr() * jac(u_d) ).cb() ;
    auto e_der2     = flatdot( jac(u_d),jac(u_d).tr(), N.tr() );
    // to do: N

    auto chi        = flat( jac(defG).tr()*jac(defN)) - flat(jac(G).tr()* jac(N)) ;
    auto chi_der    = flat( jac(defG).tr() * jac(u_n) ).cb() + flat( jac(defG).tr() * jac(u_d) ).cb() ;
    auto chi_der2   = flat( jac(u_d).tr() * jac(u_n) + jac(u_d).tr() * jac(u_n) ) ;
    // to do: M

    // E\zeta\zeta
    auto Ezz        = 0.5 * ( defN.tr() * defN - N.tr() * N );
    auto Ezz_der    = 0.5 * ( defN.tr() * u_n.tr() + defN.tr() * u_n.tr() );
    auto Ezz_der2   = 0.5 * ( u_n * u_n.tr() + u_n * u_n.tr() );

    // Gamma (all multiplied by 2)
    auto gamma      = 2 * 0.5 * ( jac(defG).tr() * defN - jac(G).tr() * N ).tr();
    auto gamma_der  = 2 * 0.5 * ( (defN.tr() * jac(u_d)).tr() + jac(defG).tr() * u_n.tr() ).tr();
    auto gamma_der2 = 2 * 0.5 * ( jac(u_d).tr() * u_n.tr() + jac(u_d).tr() * u_n.tr() );  // <-------------- Problematic

    // auto K_L        = e_der * e_der.tr() + chi_der * chi_der.tr();// + Ezz_der.tr() * Ezz_der;// + gamma_der.tr() * gamma_der;
    auto K_L        = e_der * e_der.tr() + chi_der * chi_der.tr() + Ezz_der.tr() * Ezz_der + gamma_der * gamma_der.tr();
    // auto K_NL       = e_der * e_der.tr() + chi_der * chi_der.tr() + Ezz_der.tr() * Ezz_der + gamma_der * gamma_der.tr();
    ...
    // gsDebugVar(ev.eval(e,pt));
    gsDebugVar(ev.eval(e_der,pt));
    gsDebugVar(ev.eval(e_der.tr(),pt));
    // gsDebugVar(ev.eval(e_der2,pt)); //todo

    // gsDebugVar(ev.eval(chi,pt));
    gsDebugVar(ev.eval(chi_der,pt));
    gsDebugVar(ev.eval(chi_der.tr(),pt));
    gsDebugVar(ev.eval(chi_der2,pt));

    // gsDebugVar(ev.eval(Ezz,pt));
    gsDebugVar(ev.eval(Ezz_der,pt));
    gsDebugVar(ev.eval(Ezz_der.tr(),pt));
    gsDebugVar(ev.eval(Ezz_der2,pt));

    // gsDebugVar(ev.eval(gamma,pt));
    gsDebugVar(ev.eval(gamma_der,pt));
    gsDebugVar(ev.eval(gamma_der.tr(),pt));
    gsDebugVar(ev.eval(gamma_der2,pt));


    gsDebugVar(ev.eval(K_L,pt));
