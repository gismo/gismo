...
namespace
{
    ...
    struct Options
    {
        ...
        // Opposing S-fold interface control:
        // --sfold-allow-interface-patches : allow S-fold on interface patches (default: skip them)
        // --sfold-flip-patches <p1,p2,...> : negate S-fold amplitude for these patches
        // --sfold-only-patches <p1,p2,...> : apply S-fold only to these patches (whitelist)
        bool sfoldAllowInterfacePatches = false;
        std::set<index_t> sfoldFlipPatches;
        std::set<index_t> sfoldOnlyPatches;
...
int main(int argc, char* argv[])
{
    ...
    for (int ai = 1; ai < argc; ++ai)
    {
        ...
        else if (arg == "--sfold-allow-interface-patches")
        {
            opt.sfoldAllowInterfacePatches = true;
        }
        else if ((arg == "--sfold-flip-patches") && ai + 1 < argc)
        {
            const std::string spec = argv[++ai];
            // Parse comma-separated patch indices: e.g. "5" or "4,5"
            std::istringstream ss(spec);
            std::string token;
            while (std::getline(ss, token, ','))
            {
                try {
                    const index_t p = static_cast<index_t>(std::stoi(token));
                    opt.sfoldFlipPatches.insert(p);
                } catch (...) {
                    gsWarn << "Ignoring non-integer in --sfold-flip-patches: " << token << "\n";
                }
            }
        }
        else if ((arg == "--sfold-only-patches") && ai + 1 < argc)
        {
            const std::string spec = argv[++ai];
            // Parse comma-separated patch indices: e.g. "6,8,9"
            std::istringstream ss(spec);
            std::string token;
            while (std::getline(ss, token, ','))
            {
                try {
                    const index_t p = static_cast<index_t>(std::stoi(token));
                    opt.sfoldOnlyPatches.insert(p);
                } catch (...) {
                    gsWarn << "Ignoring non-integer in --sfold-only-patches: " << token << "\n";
                }
            }
        }
        else if ((arg == "--swirl-only-patches") && ai + 1 < argc)
        {
            const std::string spec = argv[++ai];
            // Parse comma-separated patch indices: e.g. "9" or "6,8,9"
            std::istringstream ss(spec);
            std::string token;
            while (std::getline(ss, token, ','))
            {
                try {
                    const index_t p = static_cast<index_t>(std::stoi(token));
                    opt.swirlOnlyPatches.insert(p);
                } catch (...) {
                    gsWarn << "Ignoring non-integer in --swirl-only-patches: " << token << "\n";
                }
            }
        }
        else
    ...
    {
        log << "sfold-flip-patches:";
        for (index_t p : opt.sfoldFlipPatches) log << " " << p;
        log << "\n";
    }
    if (!opt.sfoldOnlyPatches.empty())
    {
        log << "sfold-only-patches:";
        for (index_t p : opt.sfoldOnlyPatches) log << " " << p;
        log << "\n";
    }
    ...
    if (nlo.sfoldAmplitude != 0.0)
    {
        ...
        // By default, S-fold is skipped for these because any distortion of their
        // shared boundary drive featureError above epsilon_f.
        // NOTE: fixedPointMask() already protects interface boundary control points,
        // so distorting the INTERIOR of interface patches is actually safe.
        // Use --sfold-allow-interface-patches to enable this.
        ...
        {
            if (opt.sfoldAllowInterfacePatches)
            {
                log << "Interface patches allowed for S-fold (--sfold-allow-interface-patches):";
        ...
        if (!opt.sfoldFlipPatches.empty())
        {
            log << "S-fold amplitude NEGATED for patches:";
            for (index_t p : opt.sfoldFlipPatches) log << " " << p;
            log << "\n";
        }
        ...
        for (index_t pIdx = 0; pIdx < static_cast<index_t>(newmp.nPatches()); ++pIdx)
        {
            if (nlo.patchIndex >= 0 && pIdx != nlo.patchIndex)
                continue;
            // If a whitelist is specified, skip patches not in the list.
            if (!opt.sfoldOnlyPatches.empty() && !opt.sfoldOnlyPatches.count(pIdx))
                continue;
            // Skip interface patches unless explicitly allowed.
            if (!opt.sfoldAllowInterfacePatches && interfacePatches.count(pIdx))
                continue;
            ...
            // For flip-patches, negate the amplitude (opposing direction).
            const real_t signMult = opt.sfoldFlipPatches.count(pIdx) ? -1.0 : 1.0;
            if (signMult < 0.0)
                gsInfo << "Patch " << pIdx << ": s-fold amplitude NEGATED (opposing direction)\n";
            ...
            const real_t safeAmplitude = tuneParameterByBisection(
                *mp,
                ...
                opt.fitGrid,
                "sfold-amplitude",
                [sfoldNlo, signMult](gsMultiPatch<>& trialMp, index_t patchIdx, real_t amp)
                {
                    applySfoldDistortion(trialMp, patchIdx, signMult * amp,
            ...
            log << "Patch " << pIdx << ": s-fold applied, effective amplitude="
                << (signMult * safeAmplitude) << " (safe magnitude=" << safeAmplitude << ")\n";
            gsInfo << "Patch " << pIdx << ": s-fold safe amplitude=" << (signMult * safeAmplitude) << "\n";
        }
    ...
    if (nlo.swirlAlpha != 0.0)
    {
        ...
        if (!opt.swirlOnlyPatches.empty())
        {
            log << "Swirl whitelist (--swirl-only-patches):";
            for (index_t p : opt.swirlOnlyPatches) log << " " << p;
            log << "\n";
        }
        ...
        for (index_t pIdx = 0; pIdx < static_cast<index_t>(newmp.nPatches()); ++pIdx)
        {
            if (nlo.patchIndex >= 0 && pIdx != nlo.patchIndex)
                continue;
            // If a whitelist is specified, skip patches not in the list.
            if (!opt.swirlOnlyPatches.empty() && !opt.swirlOnlyPatches.count(pIdx))
                continue;
            ...
            bool foundRegular = false;
            // Use newmp (current distorted state) as inputMp reference for boundary
            // restoration — not *mp (original) — so C0 consistency with already-distorted
            // adjacent patches is preserved during the bisection.
