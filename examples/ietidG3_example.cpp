...
real_t powerIteration(const gsSparseMatrix<>& A, const gsLinearOperator<>& pc, index_t N = 10)
{
    GISMO_ASSERT( A.rows() == A.cols(), "Dimensions do not argee");
    GISMO_ASSERT( pc.rows() == pc.cols(), "Dimensions do not argee");
    GISMO_ASSERT( pc.rows() == A.rows(), "Dimensions do not argee");

    gsMatrix<> x, y, z;
    x.setRandom(A.rows(),1);
    for (index_t i = 0; i < N; ++i)
    {
        const real_t v = x.col(0).dot(x.col(0));
        x /= math::sqrt( v );
        y = A*x;
        pc.apply(y, z);
        x.swap(z);
    ...
    return x.col(0).dot(y.col(0)) / z.col(0).dot(y.col(0));
}
...
template<class T>
class gsInexactIETIPrec : public gsLinearOperator<T> {
private:
...
public:
    ...
    gsInexactIETIPrec(const gsSparseMatrix<T>& K,
                      BasePtr firstprecond,
                      const gsSparseMatrix<T> &ai_mass,
                      const gsSparseMatrix<T> &mixed_mass)
    : m_fastDiag(firstprecond), m_originalSize(m_fastDiag->rows()), m_totalSize(K.rows())
    {
        m_edgeSolver = makeSparseCholeskySolver(ai_mass);

        // Construct R1T
        gsSparseEntries<> se;
        se.reserve(10*K.rows());

        for (index_t r = 0; r < m_originalSize; r++)
            se.add(r, r, 1);

        gsMatrix<T> transf;
        m_edgeSolver->apply(mixed_mass.transpose(),transf);

        for (index_t r = 0; r < transf.rows(); r++)
            for (index_t c = 0; c < transf.cols(); c++)
                if (abs(transf(r,c)) > 1e-4)
                    se.add(m_originalSize+r, c, transf(r,c) );

        m_R1T.resize(K.rows(), m_originalSize);
        m_R1T.setFrom(se);

        // Construct R2T
        m_R2T.resize(K.rows(), K.cols() - m_originalSize);
        index_t colentry = 0;
        for (index_t r = m_originalSize; r < K.rows(); r++, colentry++)
            m_R2T.insert(r, colentry) = T(1);
        m_R2T.makeCompressed();

        real_t alpha1 = powerIteration(m_R1T.transpose()*K*m_R1T, *m_fastDiag);
        DEBUGVAR(alpha1);

        real_t alpha2 = powerIteration(m_R2T.transpose()*K*m_R2T, *m_edgeSolver);
        DEBUGVAR(alpha2);

        m_R1T /= math::sqrt(alpha1);
        m_R2T /= math::sqrt(alpha2);

        //real_t totalAlpha = powerIteration(K, *this);
        //DEBUGVAR(totalAlpha);

    }

    static uPtr make(const gsSparseMatrix<T>& K,
                        BasePtr firstprecond,
                        const gsSparseMatrix <T> &ai_mass,
                        const gsSparseMatrix <T> &mixed_mass)
    {
        return uPtr(new gsInexactIETIPrec(K, give(firstprecond), ai_mass, mixed_mass));
    }
    ...
    void apply(const gsMatrix <T> &rhs, gsMatrix <T> &x) const {
        ...
        m_edgeSolver->apply(m_R2T.transpose() * rhs, solExP2);
        x.noalias() = m_R1T * solExP1 + m_R2T * solExP2;
    }

    virtual index_t rows() const { return m_totalSize; }

    virtual index_t cols() const { return m_totalSize; }
...
public:
                         ...
                         gsGenericAssembler<T> & ass)
    {
        ...
        B1.eval_into(md1.points, basisData1);
        B2.eval_into(md2.points, basisData2);
        ...
        for (index_t k = 0; k < quWeights.rows(); ++k) // loop over quadrature nodes
        {
            ...
            // Take blocks of values of basis functions
...
int main(int argc, char *argv[])
{
    ...
    gsInfo << "Run ietidG3_example with options:\n" << cmd << std::endl;
...
//! inexact stuff
        ...
        //hatassembler.assembleStiffness(0, false); //TODO
        for (size_t i=0; i<ietiMapper.artificialIfaces(k).size(); ++i)
        {
            patchSide side1(0,ietiMapper.artificialIfaces(k)[i].assignedTo.side());
            patchSide side2(i+1,ietiMapper.artificialIfaces(k)[i].takenFrom.side());
        ...
        DEBUGVAR(classical_dofs);
        DEBUGVAR(ai_dofs);

        gsSparseMatrix<> mixed_mass = hatassembler.matrix().topRows(classical_dofs).rightCols(ai_dofs);
        gsSparseMatrix<> ai_mass = hatassembler.matrix().bottomRows(ai_dofs).rightCols(ai_dofs);
        stiffness.resize( hatassembler.matrix().rows(), hatassembler.matrix().cols() );

        gsScaledDirichletPrec<>::Blocks blocks
                = gsScaledDirichletPrec<>::matrixBlocks(
                    hatassembler.matrix()+stiffness,
                    skeletonDofs);
	...
	gsInfo << "[" << localMatrix.rows() << " classical + ai dofs]";
        ...
        gsPreconditionerOp<>::Ptr localPrec = gsPreconditionerFromOp<>::make(
                                                                       makeMatrixOp(gsSparseMatrix<>(localMatrix).moveToPtr()),
                                                                       gsInexactIETIPrec<real_t>::make(localMatrix,fastdiagOp,ai_mass,mixed_mass)
                                                                       );




        localPrec->setNumOfSweeps(2); //TODO
        DEBUGMAT(*localPrec);
        #ifndef NDEBUG
        /*{
            gsMatrix<> dis;
