...
///
/// This function takes a collection of TensorBSpline patches and creates a quad
/// mesh where each face corresponds to one patch. The corners of each face are
/// positioned at the corner control points of the corresponding BSpline.
/// Adjacent patches are automatically connected by detecting shared corner
/// control points.
///
/// Each face gets a property "bezier_points" containing a
/// gsFreeformFaceData<N,3> object with the control points from the original
/// BSpline and a back reference to the face. The control points are oriented
/// such that the (0,0) to (0,N-1) first row aligns with the first halfedge of
/// the face.
///
...
template <size_t N>
gsSurfMesh bsplinesToSurfMesh(
    const std::vector<std::unique_ptr<gsTensorBSpline<2, real_t>>>& patches)
{
    gsSurfMesh mesh;

    // Add face property for storing gsFreeformFaceData with control points
    auto bezier_points =
        mesh.add_face_property<gsFreeformFaceData<N, 3>>("bezier_points");
        ...
        [&](const gsMatrix<real_t>& point) -> gsSurfMesh::Vertex
    {
        ...
        {
            gsSurfMesh::Vertex v = mesh.add_vertex(
                gsSurfMesh::Point(point(0), point(1), point(2)));
            cornerMap[key] = v;
            return v;
        }
    };
