/** @file cahn-hilliard.cpp
    ...
    Tensor product (check flags in .xml file): with lambda = 0.05
    //  =========== QUADRATIC BASIS ===========
    ./bin/ch_sigmoid_time_cosine_load -r 6 -t 0.075 -N 40 -c 0 -v 2 -s 0 -e 1 --plot  
            ...
            std::string out)
{
    ...
    if (MESHopt.askSwitch("Adaptive",true)) // load basis from file
    {
        gsFileData<> fd1;
    ...
    gsFunctionExpr<> sourceQold("800*tanh(20*x + 2*cos((pi*t)/3) - 10)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1) + 125600*tanh(20*x + 2*cos((pi*t)/3) - 10)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1)^2 + 61600*tanh(20*x + 2*cos((pi*t)/3) - 10)^3*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1) + (2*pi*sin((pi*t)/3)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1))/3",2);
    gsFunctionExpr<> sourceQnew("800*tanh(20*x + 2*cos((pi*t)/3) - 10)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1) + 125600*tanh(20*x + 2*cos((pi*t)/3) - 10)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1)^2 + 61600*tanh(20*x + 2*cos((pi*t)/3) - 10)^3*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1) + (2*pi*sin((pi*t)/3)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1))/3",2);
    ...
    gsFunctionExpr<> bc1("1540*tanh(20*x + 2*cos((pi*t)/3) - 10)^2*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1) + 800*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1)^2 + 20*tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 20","0",2);
    // (2) Laplace boundary condition
    gsFunctionExpr<> bc2("800*tanh(20*x + 2*cos((pi*t)/3) - 10)*(tanh(20*x + 2*cos((pi*t)/3) - 10)^2 - 1)",2);
    // (3) Grad c(x,t)
    gsFunctionExpr<> bc3("20 - 20*tanh(20*x + 2*cos((pi*t)/3) - 10)^2","0",2);
    // =====================================================
...
#   else
            ...
            for (index_t dt_it = 0; dt_it != lmax; dt_it++)
            {
                ...
                for (index_t k = 0; k!=2; k++)
                {
                    ...
                    for (index_t it = 0; it!= maxIt; it++)
                    {
                        ...
                        // ==== Assemble boundary terms (manufactured)????? TO BE CORRECTED ====
                        ...
                        auto bc_Laplace = A.getCoeff(bc2,G); // Laplace of the solution c at alpha time
                        bc3.set_t(alpha_time);
                        auto bc_gradc = A.getCoeff(bc3, G); // gradient of c at alpha time

                        auto expr1 = lambda * (igrad(w,G) * nv(G)) * bc_Laplace; // transpose to have space 1?
                        auto expr2 = penalty * (igrad(w,G) * nv(G).normalized()) * hmax * (bc_gradc.tr() * nv(G));
                        auto expr3 = lambda * ilapl(w,G) * (bc_gradc.tr() * nv(G));
    ...
    // gsSparseSolver<>::PardisoLU solverpLU;
            ...
            // Doing the evaluation of the manufactured solution before the mesh adaptivity
            // to avoid assert in gsExpressions.h (line 1429) 
            ...
            if (MESHopt.askSwitch("Adaptive",true))
            {
                ...
                // gsInfo << "============ REFINEMENT =============\n";
                // gsInfo << "cInt min = " << cvec.minCoeff() << "\n";
                // gsInfo << "cInt max = " << cvec.maxCoeff() << "\n";
                // gsInfo << "Max area: " << avec.maxCoeff() << "\n";
                // gsInfo << "Min area: " << avec.minCoeff() << "\n";
                // gsInfo << "=====================================\n";
    ...
    // =====================================
     ...
     auto u_manufactured = ev.getVariable(source_time, G);
     gsInfo << "Maximum value solution: " << mp_cnew.patch(0).coefs().maxCoeff() << "\n";
     gsInfo << "Minimum value solution: " << mp_cnew.patch(0).coefs().minCoeff() << "\n";
     auto mp_var = ev.getVariable(mp_cnew, G);
     // ==========================
