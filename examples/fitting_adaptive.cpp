...
int main(int argc, char *argv[])
{
    ...
    ref.computeApproximation(maxPcIter);
    ...
    std::vector<real_t> AvgErrPerCell;
    std::vector<gsMatrix<real_t>> R_cells;
    std::vector<gsMatrix<real_t>> AD_cells;
    std::vector<index_t> AD_boxes;
    std::string output_name = "LvL";
    ...
    for(int i = 0; i <= iter; i++)
    {
        ...
        real_t toll = tolerance/std::pow(2, i);
        if (constantRefT ){
            toll = tolerance;
        }
        std::string prefix = output_name + internal::to_string(i+1);
        time.restart();
        R_cells = getAllCellsFromBasis(THB);
        if ( CellAvgRef ){
            AvgErrPerCell= ref.cellAvgErr(errors, uv);
        }
        else{
            AvgErrPerCell= ref.cellMaxErr(errors, uv);
        }
        gsInfo<<"The "<< i << "-th hierarchical domain has "<<AvgErrPerCell.size()<<" cells.\n";
        gsInfo<<"Cells with error grater than " << toll <<"\n";

        for(index_t avg_iter=0; avg_iter<AvgErrPerCell.size(); avg_iter++)
        {
            if(AvgErrPerCell[avg_iter] >= toll )
            {
                gsInfo<<"Cell ("<< avg_iter <<"), error: " <<AvgErrPerCell[avg_iter]<<"\n";
                AD_cells.push_back(R_cells[avg_iter]);
            }
        }

        ref.appendBox(AD_cells, AD_boxes);
        THB.refineElements(AD_boxes);
        ref.computeApproximation(maxPcIter);
        time.stop();

        gsMesh<> Ref_mesh(THB);
        gsWriteParaview(Ref_mesh, "AdaptRef_mesh");
        ...
        gsInfo<<"MSE    error : "<< sum_of_2errors/errors2.size() <<"\n";
