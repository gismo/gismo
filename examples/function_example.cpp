...
int main(int argc, char* argv[])
{
    ...
    gsFunctionExpr<> fun1("x","y",2);
    gsFunctionExpr<> fun2("x^2","y^2","x*y",2);
    gsFunctionExpr<> fun3("sqrt(x)","sqrt(y)","z/(sqrt(x)*sqrt(y))",3);
    std::vector<gsFunction<>*> functions{&fun1,&fun2,&fun3};

    gsFunctionComposition<> composition(functions);
    gsDebugVar(composition.domainDim());
    gsDebugVar(composition.targetDim());
    gsDebugVar(composition);

    gsMatrix<> points(2,3);
    points.col(0).setConstant(0.0);
    points.col(1).setConstant(0.5);
    points.col(2).setConstant(1.0);

    gsMatrix<> result;
    composition.deriv_into(points,result);

    gsDebugVar(composition.eval(points));
    gsDebugVar(result);
